...
export function createBillingApi(ctx: DemoContext): BillingApi {
  return {
    getEntitlement: () =>
      ctx.run((): EntitlementState => {
        const s = ctx.store.state;
        const active = s.subscriptions.find(
          (sub) =>
            sub.status === 'active' || sub.status === 'trial' || sub.status === 'referral_bonus',
        );
