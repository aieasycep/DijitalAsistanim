...
describe('data source mode', () => {
  it('never allows demo in production', () => {
    expect(resolveMode({ requested: 'demo', isProduction: true, hasSupabase: true })).toBe('supabase');
    expect(resolveMode({ requested: 'demo', isProduction: true, hasSupabase: false })).toBe('supabase');
  });
  it('honours explicit request in development', () => {
    ...
    expect(resolveMode({ requested: 'supabase', isProduction: false, hasSupabase: true })).toBe('supabase');
  });
  it('falls back to demo when no backend configured', () => {
    ...
    expect(resolveMode({ requested: null, isProduction: false, hasSupabase: true })).toBe('supabase');
  });
});
