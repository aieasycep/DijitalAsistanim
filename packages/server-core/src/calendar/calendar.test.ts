...
describe('calendar · conflicts and back-to-back', () => {
  it('detects overlaps in minutes and skips all-day, cancelled and declined events', () => {
    const a = event({
      ...
      endAt: at(today, '15:00'),
    });
    const b = event({
      id: 'b',
      title: 'Doktor randevusu',
      startAt: at(today, '14:30'),
      endAt: at(today, '15:15'),
    });
    const allDay = event({
      id: 'c',
      title: 'Tatil',
      startAt: at(today, '00:00'),
      endAt: at(tomorrow, '00:00'),
      allDay: true,
    });
    ...
    const conflicts = detectConflicts([b, a, allDay, cancelled, declined], {
      userEmail: 'yunus@example.com',
    });
  ...
  it('does not report the same event mirrored from two calendars', () => {
    const a = event({
      id: 'a',
      title: 'Haftalık ekip',
      startAt: at(today, '09:00'),
      endAt: at(today, '10:00'),
      externalEventId: 'x',
    });
    const b = event({
      id: 'b',
  ...
  it('flags back-to-back meetings with less than the minimum gap', () => {
    ...
    const c = event({
      id: 'c',
      title: 'C',
      startAt: at(today, '10:35'),
      endAt: at(today, '11:00'),
    });
    const d = event({
      id: 'd',
      title: 'D',
      startAt: at(today, '11:30'),
      endAt: at(today, '12:00'),
    });
...
describe('calendar · free blocks', () => {
  ...
  it('clips to now (rounded up to 5 minutes) and returns the whole window for an empty day', () => {
    ...
    expect(blocks).toEqual([
      { startAt: at(today, '10:05'), endAt: at(today, '18:00'), minutes: 475 },
    ]);
    expect(freeBlocks([], { date: tomorrow, timezone: tz, now })).toEqual([
      { startAt: at(tomorrow, '09:00'), endAt: at(tomorrow, '18:00'), minutes: 540 },
    ]);
  });
...
describe('calendar · suggestions', () => {
  it('proposes a prep block right before a meeting when the time is free', () => {
    ...
    expect(
      suggestPrepTime(
        meeting,
        [{ startAt: at(today, '09:00'), endAt: at(today, '10:00'), minutes: 60 }],
        { timezone: tz },
      ),
    ).toBeNull();
  ...
  it('places an open task into tomorrow’s 2,5 hour gap with the canonical Turkish copy', () => {
    ...
    const en = scheduleSuggestions({
      tasks: [task({ title: 'Teklif hazırlama' })],
      commitments: [],
      freeBlocks: blocks,
      events: [],
      now,
  ...
  it('uses promises when there is no task, never re-uses an item and caps the list', () => {
    ...
    const out = scheduleSuggestions({
      tasks: [],
      commitments: [
        commitment({ id: 'c1', text: "Mehmet'e teklif gönder", dueAt: at(tomorrow, '18:00') }),
        commitment({ id: 'c2', text: 'Rapor', direction: 'other_owes' }),
      ],
      freeBlocks: blocks,
      events: [],
      now,
  ...
  it('adds prep for meetings with people and a buffer after back-to-back meetings', () => {
    ...
    expect(buffer).toMatchObject({
      title: '09:00 ve 10:00 toplantıların arka arkaya.',
      targetEventId: 'b',
      proposedStartAt: at(today, '10:10'),
      proposedEndAt: at(today, '10:40'),
    });
    expect(buffer?.detail).toBe(
      "Arada mola yok; Ürün gözden geçirme toplantısını 10:10'a kaydırmayı önerebilirim.",
    );
  });
...
describe('calendar · conflict options', () => {
  it('recommends moving the event that affects the fewest people, then alternatives, then keep', () => {
    ...
    const options = resolveConflictOptions(conflict, blocks, {
      timezone: tz,
      userEmail: 'yunus@example.com',
    });
    ...
    expect(options[0]).toMatchObject({
      ...
      icon: 'auto_awesome',
      title: "“Doktor randevusu” etkinliğini 15:00'e al",
    });
    expect(options[0]?.subtitle).toBe('Önerilen · 15:00–15:30 boş görünüyor.');
    expect(options[0]?.suggestion).toMatchObject({
      kind: 'move_event',
      targetEventId: 'b',
      proposedStartAt: at(today, '15:00'),
      proposedEndAt: at(today, '15:30'),
    });
    expect(options[1]).toMatchObject({
      title: "“Müşteri toplantısı” için 12:00'ye öner",
      subtitle: "Mehmet Yılmaz'a öneri maili taslağı hazırlanır",
    });
    expect(options[2]).toMatchObject({
      title: '“Müşteri toplantısı” etkinliğini 30 dk kısalt',
      subtitle: '14:00–14:30 · Sonrakine zamanında yetişirsin',
    });
    expect(options[3]).toMatchObject({
      kind: 'keep',
      title: 'Böyle kalsın',
      subtitle: 'Bu çakışmayı bir daha gösterme',
      needsFurtherStep: false,
      suggestion: null,
    });
    for (const o of options)
      expect(o.suggestion === null || o.suggestion.kind === 'move_event').toBe(true);
  });
...
describe('calendar · routes and leave-by', () => {
  ...
  it('calls the Routes API with a field mask and parses durations', async () => {
    ...
    expect(body).toMatchObject({
      origin: { address: 'Kadıköy' },
      destination: { address: 'Nişantaşı' },
      travelMode: 'DRIVE',
  ...
  it('returns null without routes and throws AppError on provider errors', async () => {
    ...
    await expect(failing.computeRoute({ origin: 'A', destination: 'B' })).rejects.toMatchObject({
  ...
  it('createRoutesProvider returns null for none / missing key', () => {
    ...
    expect(
      createRoutesProvider({ provider: 'google', apiKey: 'k', fetch: fetchOk({}).fetchFn })?.name,
    ).toBe('google_routes');
  });
  it('leaveByTime subtracts travel and buffer; null without location, origin or provider', async () => {
    const provider: RoutesProvider = {
      name: 'stub',
      computeRoute: async () => ({ durationMinutes: 38, distanceMeters: null, provider: 'stub' }),
    };
    const doctor = event({
      title: 'Doktor randevusu',
      startAt: at(today, '13:30'),
      endAt: at(today, '14:00'),
      location: 'Nişantaşı',
    });
    const leave = await leaveByTime(doctor, provider, { origin: 'Kadıköy', bufferMinutes: 2 });
    expect(leave).toEqual({
      leaveAt: at(today, '12:50'),
      arriveBy: at(today, '13:28'),
      durationMin: 38,
      provider: 'stub',
    });
    ...
    const failing: RoutesProvider = {
      name: 'f',
      computeRoute: async () => Promise.reject(new Error('down')),
    };
...
describe('calendar · writes and plan day', () => {
  it('resolveCalendarWrite detects remote changes', () => {
    expect(
      resolveCalendarWrite({
        expectedProviderUpdatedAt: '2026-09-01T10:00:00Z',
        remoteProviderUpdatedAt: '2026-09-01T10:00:00Z',
      }),
    ).toBe('apply');
    expect(
      resolveCalendarWrite({
        expectedProviderUpdatedAt: '2026-09-01T10:00:00Z',
        remoteProviderUpdatedAt: '2026-09-02T10:00:00Z',
      }),
    ).toBe('conflict');
    expect(
      resolveCalendarWrite({
        expectedProviderUpdatedAt: '2026-09-02T10:00:00Z',
        remoteProviderUpdatedAt: '2026-09-01T10:00:00Z',
      }),
    ).toBe('apply');
    expect(
      resolveCalendarWrite({
        expectedProviderUpdatedAt: null,
        remoteProviderUpdatedAt: '2026-09-02T10:00:00Z',
      }),
    ).toBe('apply');
  ...
  it('buildPlanDay filters everything to the local date and computes blocks and warnings', () => {
    ...
    const conflict = detectConflicts([
      a,
      event({ id: 'z', title: 'Z', startAt: at(today, '09:30'), endAt: at(today, '09:45') }),
    ])[0];
    ...
    expect(day.freeBlocks[0]).toEqual({
      startAt: at(today, '10:30'),
      endAt: at(today, '18:00'),
      minutes: 450,
    });
  });
});
