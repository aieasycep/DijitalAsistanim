...
describe('memory · chunks', () => {
  it('builds an email chunk from summary, key points and a short excerpt (no quoted history)', () => {
    const t = thread({
      ...
      analysis: analysis({
        ...
        keyPoints: ['Revize fiyat', 'PDF formatı'],
      }),
    });
    const [c] = buildMemoryChunks({
      source: { kind: 'email_thread', entity: t, bodyText: body, contactId: 'c-ahmet' },
      timezone: tz,
      userEmails: ['yunus@example.com'],
      retentionDays: 90,
    });
    ...
    expect(c).toMatchObject({
      ...
      contactId: 'c-ahmet',
      occurredAt: t.lastMessageAt,
      hasEmbedding: false,
    });
    expect(c.tokenCount).toBeGreaterThan(20);
    expect(c.expiresAt).toBe(
      new Date(Date.parse(t.lastMessageAt) + 90 * 24 * 3600 * 1000).toISOString(),
    );
  ...
  it('caps the excerpt at 600 characters', () => {
    ...
    const [c] = buildMemoryChunks({
      source: {
        kind: 'email_thread',
        entity: thread({ subject: 'Uzun', importance: 'high' }),
        bodyText: long,
      },
      timezone: tz,
    });
  ...
  it('skips promotions, newsletters and low importance mails without action', () => {
    const promo = thread({
      subject: 'İndirim',
      category: 'promotion',
      analysis: analysis({ summary: 'x', category: 'promotion' }),
    });
    const newsletter = thread({
      subject: 'Bülten',
      labels: ['INBOX', 'CATEGORY_PROMOTIONS'],
      analysis: analysis({ summary: 'x' }),
    });
    const lowInert = thread({
      subject: 'Bilgi',
      importance: 'low',
      analysis: analysis({ summary: 'x', importance: 'low' }),
    });
    const lowActionable = thread({
      subject: 'Fatura',
      importance: 'low',
      analysis: analysis({
        summary: 'Fatura 1.842 TL',
        importance: 'low',
        requiresUserAction: true,
      }),
    });
    for (const t of [promo, newsletter, lowInert])
      expect(
        buildMemoryChunks({
          source: { kind: 'email_thread', entity: t, bodyText: body },
          timezone: tz,
        }),
      ).toEqual([]);
    expect(
      buildMemoryChunks({ source: { kind: 'email_thread', entity: lowActionable }, timezone: tz }),
    ).toHaveLength(1);
    expect(
      buildMemoryChunks({
        source: { kind: 'email_thread', entity: thread({ subject: 'Boş' }) },
        timezone: tz,
      }),
    ).toEqual([]);
  });
  it('builds chunks for events, commitments, captures and meeting notes', () => {
    ...
    const [ec] = buildMemoryChunks({
      source: { kind: 'calendar_event', entity: event },
      timezone: tz,
    });
    expect(ec?.content).toBe(
    ...
    expect(ec).toMatchObject({
      ...
      topic: 'Mehmet ile müşteri toplantısı',
    ...
    expect(
      buildMemoryChunks({
        source: { kind: 'calendar_event', entity: { ...event, status: 'cancelled' } },
        timezone: tz,
      }),
    ).toEqual([]);
    ...
    const [cc] = buildMemoryChunks({
      source: { kind: 'commitment', entity: commitment },
      timezone: tz,
    });
    expect(cc?.content).toBe(
      "Verdiğin söz: Mehmet'e teklif gönder\n“yarın göndereceğim”\nSon tarih: 6 Eylül 2026 18:00 (yarın)",
    );
    expect(cc).toMatchObject({
      sourceType: 'meeting_note',
      personName: 'Mehmet Yılmaz',
      occurredAt: at('2026-09-01', '15:31'),
    });
    ...
    const capture: Capture = {
      ...
      kind: 'image',
      status: 'analyzed',
      extractedText: 'Fatura no 123456 tutar 1.842,00 TL son ödeme 10.09.2026',
      analysis: {
    ...
    const note: PostMeetingNote = {
      ...
      createdAt: at('2026-09-01', '15:31'),
      updatedAt: now,
    };
    const [nc] = buildMemoryChunks({
      source: {
        kind: 'meeting_note',
        entity: note,
        eventTitle: 'Mehmet ile müşteri toplantısı',
        personName: 'Mehmet Yılmaz',
        contactId: 'c-mehmet',
        eventAt: at('2026-09-01', '14:30'),
      },
      timezone: tz,
    });
    expect(nc?.content).toBe(
      "Toplantı: Mehmet ile müşteri toplantısı\nMehmet'e yarın teklif göndereceğim.",
    );
    expect(nc).toMatchObject({
      sourceType: 'meeting_note',
      personName: 'Mehmet Yılmaz',
      contactId: 'c-mehmet',
      occurredAt: at('2026-09-01', '14:30'),
    });
...
describe('memory · full-text query', () => {
  it('drops Turkish stopwords and question particles, keeps phrases and negations', () => {
    ...
    expect(buildFtsQuery('"fiyat teklifi" -kampanya veya fatura')).toBe(
      '"fiyat teklifi" -kampanya or fatura',
    );
  ...
  it('ranks by score blended with recency and respects the token budget', () => {
    const chunks = [
      chunk({
        ...
        score: 0.95,
        occurredAt: at('2026-06-01', '10:00'),
      }),
      chunk({
        id: 'new-mid',
        content: 'b'.repeat(400),
        score: 0.6,
        occurredAt: at('2026-09-05', '07:00'),
      }),
      chunk({
        id: 'big',
        content: 'c'.repeat(4000),
        score: 0.9,
        occurredAt: at('2026-09-04', '07:00'),
      }),
      chunk({
        id: 'small',
        content: 'd'.repeat(40),
        score: 0.3,
        occurredAt: at('2026-09-01', '07:00'),
      }),
      chunk({
        id: 'small',
        content: 'd'.repeat(40),
    ...
    const recencyOnly = rankAndTrimContext(
      chunks.map((c) => ({ ...c, score: undefined })),
      { maxTokens: 10_000, now },
    );
...
describe('memory · grounding', () => {
  const bill = chunk({
    id: 'm-bill',
    content: 'CK Enerji elektrik faturası: 1.842,00 TL, son ödeme tarihi 10 Eylül 2026.',
  });
  const flight = chunk({
    id: 'm-flight',
    content:
      'THY TK2412 uçuşu: İstanbul (IST) → Antalya (AYT), 2026-09-06 09:15–10:30. PNR ABC123. Online check-in açık.',
  });
  it('extracts and canonicalises facts', () => {
    ...
    const facts = extractFacts(
      'Fatura 1.842 TL, son ödeme 10 Eylül saat 14:30, TK2412, %8 indirim, 3 gün.',
    );
  ...
  it('accepts answers whose facts are in the cited chunks', () => {
    const res = groundingCheck(
      'Elektrik faturası 1.842 TL, son ödeme 10 Eylül.',
      ['m-bill'],
      [bill, flight],
    );
    expect(res).toEqual({ uncertain: false, unsupportedFacts: [] });
    const res2 = groundingCheck(
      "TK2412 uçuşun yarın 09:15'te kalkıyor, PNR ABC123. 6 Eylül'de Antalya'da olacaksın.",
      ['m-flight'],
      [bill, flight],
    );
  ...
  it('flags invented numbers, dates, times and amounts', () => {
    const res = groundingCheck(
      'Fatura 2.000 TL, son ödeme 12 Eylül 14:00; 3 gün kaldı.',
      ['m-bill'],
      [bill, flight],
    );
    ...
    expect(
      groundingCheck('Uçuş TK2412 saat 10:00.', ['m-bill'], [bill, flight]).unsupportedFacts,
    ).toEqual(['10:00', 'TK2412']);
  });
  it('is uncertain without citations (unless the answer is generic) or with unknown citations', () => {
    expect(groundingCheck('Fatura 1.842 TL.', [], [bill])).toEqual({
      uncertain: true,
      unsupportedFacts: ['1.842 TL'],
    });
    expect(groundingCheck('Teklif gönderildi, yanıt bekleniyor.', [], [bill])).toEqual({
      uncertain: false,
      unsupportedFacts: [],
    });
...
describe('memory · source refs and search results', () => {
  it('buildSourceRefs dedupes per source and adds a citation excerpt', () => {
    const a = chunk({
      ...
      source: { type: 'gmail', id: 'e1', label: 'Gmail', person: 'Ahmet Yılmaz', timestamp: now },
    });
    const b = chunk({
      id: 'b',
      content: 'Aynı kaynak',
      source: { type: 'gmail', id: 'e1', label: 'Gmail', timestamp: now },
    });
    const c = chunk({
      id: 'c',
    ...
    expect(refs[0]).toMatchObject({
      ...
      id: 'e1',
      person: 'Ahmet Yılmaz',
      excerpt: 'Birinci parça. İkinci satır.',
    });
  ...
  it('maps chunks and entities to SearchResult, best first, deduped and limited', () => {
    const contact: Contact = {
      ...
      company: 'Müşteri Ltd.',
      title: 'Genel Müdür',
      lastContactAt: at('2026-09-01', '15:31'),
      interactionCount: 42,
      isVip: true,
    ...
    const t = thread({
      id: 'e4',
      subject: 'Re: Teklif v2',
      participants: [{ name: 'Mehmet Yılmaz', email: 'mehmet@musteri.com' }],
      lastMessageAt: at('2026-09-02', '10:15'),
      analysis: analysis({ summary: "Mehmet'e Teklif v2 gönderildi; geri bildirim bekleniyor." }),
    });
    const m = chunk({
      id: 'm1',
      content: "Mehmet Yılmaz'a Teklif v2 PDF gönderildi; geri bildirim bekleniyor.",
      topic: 'Teklif v2',
      sourceId: 'e4',
      score: 0.92,
      occurredAt: at('2026-09-02', '10:15'),
    });
    const dup = chunk({
      id: 'm2',
      content: 'Teklif v2 tekrar',
      topic: 'Teklif v2',
      sourceId: 'e4',
      score: 0.5,
      occurredAt: at('2026-09-01', '10:15'),
    });
    const results = toSearchResults(
      { chunks: [m, dup], threads: [t], contacts: [contact] },
      { mode: 'semantic', query: 'Mehmet teklif', now, limit: 10 },
    );
    ...
    expect(results[2]).toMatchObject({
      title: 'Mehmet Yılmaz',
      summary: 'Genel Müdür · Müşteri Ltd.',
      entityId: 'c-mehmet',
    });
    ...
    expect(
      toSearchResults(
        { chunks: [m, dup], threads: [t] },
        { mode: 'fts', query: 'teklif', now, limit: 1 },
      ),
    ).toHaveLength(1);
