...
export function voiceIntent(input: VoiceIntentInput): PromptSpec<VoiceIntentAi> {
  ...
  const screens = input.screens?.length
    ? input.screens.map((s) => clipInline(s, 40))
    : [...DEFAULT_VOICE_SCREENS];
  const system = composeSystem({
    ...
    rules: [
      ...
      en
        ? `navigateTo must be one of: ${screens.join(', ')}; null for any other intent.`
        : `navigateTo şu değerlerden biri olmalı: ${screens.join(', ')}; diğer niyetlerde null.`,
      en
        ? 'question: the cleaned-up question text when intent=question.'
        : 'question: intent=question ise temizlenmiş soru metni.',
      en
    ...
    sections: [
      {
        ...
        body: joinLines([
          ...
          input.contactName
            ? `${en ? 'This voice session is about' : 'Bu sesli oturum şu kişi hakkında'}: ${clipInline(input.contactName, 120)}`
            : null,
        ]),
      },
    ],
