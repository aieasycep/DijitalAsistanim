...
function request<T extends ApprovalActionType>(
  type: T,
  payload: ApprovalPayloadMap[T],
  extra: Partial<CreateApprovalInput<T>> = {},
): CreateApprovalInput<T> {
  return {
    type,
    what: 'Ahmet Yılmaz’a yanıt gönder',
    why: 'Teklif için dönüş bekliyor',
    payload,
...
async function pendingEmail(): Promise<ApprovalAction<'email_send'>> {
  return createApproval(request('email_send', emailPayload), {
    userId: 'u1',
    now,
    timezone: tz,
    provider: 'google',
...
describe('approvals · state machine', () => {
  ...
  it('transition stamps timestamps, counts attempts and throws conflict otherwise', async () => {
    ...
    const executed = transition(retry, 'executed', {
      now: '2026-09-04T06:00:04.000Z',
      executionResult: { messageId: 'gm-1' },
    });
...
describe('approvals · createApproval', () => {
  it('validates, keeps the original payload, computes a stable idempotency key and scope', async () => {
    ...
    expect(a.changeSummary).toEqual([
      'Kime: Ahmet Yılmaz',
      'Bilgi: zeynep@firma.com',
      'Konu: Re: Revize teklif',
    ]);
    expect(a.id).toMatch(/^[0-9a-f-]{36}$/);

    const again = await createApproval(
      request('email_send', { ...emailPayload, bodyText: 'Farklı bir taslak.' }),
      { userId: 'u1', now, timezone: tz },
    );
    expect(again.idempotencyKey).toBe(a.idempotencyKey);
    const otherUser = await createApproval(request('email_send', emailPayload), {
      userId: 'u2',
      now,
      timezone: tz,
    });
    expect(otherUser.idempotencyKey).not.toBe(a.idempotencyKey);
    const explicit = await createApproval(
      request('email_send', emailPayload, { idempotencyKey: 'client:abcdef123456' }),
      { userId: 'u1', now, timezone: tz },
    );
  ...
  it('rejects invalid payloads with a validation error listing issues', async () => {
    ...
    await expect(
      createApproval(request('email_send', bad), { userId: 'u1', now, timezone: tz }),
    ...
    await expect(
      createApproval(request('calendar_create', badCal), { userId: 'u1', now, timezone: tz }),
    ).rejects.toMatchObject({ code: 'validation' });
  });

  it('honours a custom TTL and generates summaries in English when asked', async () => {
    const a = await createApproval(request('email_send', emailPayload), {
      userId: 'u1',
      now,
      timezone: tz,
      ttlHours: 24,
...
describe('approvals · applyEdit', () => {
  it('validates, marks editedByUser and regenerates the summary while keeping the original', async () => {
    ...
    const edited = applyEdit(
      a,
      { ...emailPayload, subject: 'Re: Revize teklif (güncel)', cc: [] },
      { now: '2026-09-04T06:00:00.000Z', timezone: tz },
    );
    ...
    expect(edited.changeSummary).toEqual([
      'Kime: Ahmet Yılmaz',
      'Konu: Re: Revize teklif (güncel)',
    ]);
  ...
  it('refuses edits on non-pending approvals, invalid payloads and account retargeting', async () => {
    ...
    expect(() => applyEdit(approved, emailPayload, { now, timezone: tz })).toThrow(
      'Yalnızca bekleyen işlemler düzenlenebilir.',
    );
    expect(() => applyEdit(a, { ...emailPayload, subject: '' }, { now, timezone: tz })).toThrow(
      AppError,
    );
    try {
      applyEdit(
        a,
        { ...emailPayload, accountId: '44444444-4444-4444-8444-444444444444' },
        { now, timezone: tz },
      );
      throw new Error('should have thrown');
    } catch (e) {
...
describe('approvals · summarizeChange', () => {
  it('renders calendar creation with relative Turkish day labels and the calendar name', () => {
    const lines = summarizeChange(
      'calendar_create',
      {
        ...
        startAt: '2026-09-05T06:10:00.000Z',
        endAt: '2026-09-05T07:00:00.000Z',
        location: 'Levent ofis',
        attendees: [{ email: 'ahmet@musteri.com' }, { email: 'zeynep@firma.com' }],
      },
      { timezone: tz, now, provider: 'google' },
    );
    expect(lines).toEqual([
      'Başlık: Teklif görüşmesi',
      'Ne zaman: Yarın 09:10–10:00',
      'Nerede: Levent ofis',
      'Katılımcı: 2 kişi',
      'Takvim: Google',
    ]);
  ...
  it('renders all-day events, updates, tasks, reminders and commitments (tr + en)', () => {
    const allDay = summarizeChange(
      ...
      { timezone: tz, now, provider: 'device' },
    );
    expect(allDay).toEqual([
      'Başlık: Bayram',
      'Ne zaman: Pazartesi (tüm gün)',
      'Takvim: Cihaz takvimi',
    ]);
    ...
    expect(update).toEqual([
      'New time: Today 14:00–14:30',
      'New location: Zoom',
      'Description will be updated',
      'Calendar: Microsoft',
    ]);

    const task = summarizeChange(
      'task_create',
      { accountId, title: 'Teklifi revize et', dueAt: '2026-09-08T14:00:00.000Z' },
      { timezone: tz, now, provider: 'google' },
    );
    expect(task).toEqual([
      'Görev: Teklifi revize et',
      'Son tarih: Salı 17:00',
      'Liste: Google Tasks',
    ]);
    const internalTask = summarizeChange(
      'task_create',
      { title: 'Fatura öde' },
      { timezone: tz, now },
    );
    ...
    const reminder = summarizeChange(
      'reminder_create',
      { title: 'Ahmet’i ara', remindAt: '2026-09-04T16:00:00.000Z', option: 'this_evening' },
      { timezone: tz, now },
    );
    expect(reminder).toEqual(['Hatırlatıcı: Ahmet’i ara', 'Ne zaman: Bugün 19:00']);

    const commitment = summarizeChange(
      'commitment_create',
      {
        text: 'Mehmet’e teklif gönder',
        direction: 'user_owes',
        counterpartName: 'Mehmet Kaya',
        dueText: 'haftaya',
      },
      { timezone: tz, now },
    );
    expect(commitment).toEqual([
      'Sözün: Mehmet’e teklif gönder',
      'Kime: Mehmet Kaya',
      'Son tarih: haftaya',
    ]);
    const expected = summarizeChange(
      'commitment_create',
      {
        text: 'Sözleşme taslağı',
        direction: 'other_owes',
        counterpartName: 'Ayşe',
        dueAt: '2026-09-11T08:00:00.000Z',
      },
      { timezone: tz, now, locale: 'en' },
    );
    expect(expected).toEqual([
      'Expected: Sözleşme taslağı',
      'From: Ayşe',
      'Due: 11 September 11:00',
    ]);
  });
});
...
describe('approvals · scopes & execution plans', () => {
  ...
  it('plans provider-specific execution for approved actions', async () => {
    ...
    expect(planExecution(a, { provider: 'microsoft', kinds: ['email'] }, { now }).kind).toBe(
      'graph_send',
    );

    const cal = transition(
      await createApproval(
        request('calendar_create', {
          accountId,
          title: 'Toplantı',
          startAt: '2026-09-05T06:10:00.000Z',
          endAt: '2026-09-05T07:00:00.000Z',
    ...
    expect(planExecution(cal, { provider: 'google', kinds: ['calendar'] }, { now }).kind).toBe(
      'gcal_create',
    );
    expect(planExecution(cal, { provider: 'microsoft', kinds: ['calendar'] }, { now }).kind).toBe(
      'graph_event_create',
    );
    ...
    expect(planExecution(upd, { provider: 'google', kinds: ['calendar'] }, { now }).kind).toBe(
      'gcal_update',
    );
    expect(planExecution(upd, { provider: 'apple', kinds: ['calendar'] }, { now }).kind).toBe(
      'device_event_update',
    );

    const task = transition(
      await createApproval(request('task_create', { accountId, title: 'Görev' }), {
        userId: 'u1',
        now,
        timezone: tz,
      }),
    ...
    expect(
      planExecution(task, { provider: 'google', kinds: ['email', 'tasks'] }, { now }).kind,
    ).toBe('gtasks_create');
    expect(planExecution(task, { provider: 'microsoft', kinds: ['tasks'] }, { now }).kind).toBe(
      'graph_task_create',
    );
    expect(planExecution(task, { provider: 'google', kinds: ['email'] }, { now })).toMatchObject({
      kind: 'internal_task',
      requiredScope: null,
    });
    const localTask = transition(
      await createApproval(request('task_create', { title: 'Yerel görev' }), {
        userId: 'u1',
        now,
        timezone: tz,
      }),
    ...
    const rem = transition(
      await createApproval(
        request('reminder_create', {
          title: 'Ara',
          remindAt: '2026-09-04T16:00:00.000Z',
          option: 'custom',
        }),
        { userId: 'u1', now, timezone: tz },
      ),
    ...
    const com = transition(
      await createApproval(request('commitment_create', { text: 'Söz', direction: 'user_owes' }), {
        userId: 'u1',
        now,
        timezone: tz,
      }),
    ...
    expect(planExecution(com, { provider: 'google', kinds: ['email'] }, { now }).kind).toBe(
      'internal_commitment',
    );
  });
  ...
  it('never plans execution for pending, expired or account-less provider actions', async () => {
    ...
    expect(() => planExecution(pending, { provider: 'google', kinds: ['email'] }, { now })).toThrow(
      'İşlem onaylanmadan çalıştırılamaz.',
    );
    const approved = transition(pending, 'approved', { now });
    expect(() =>
      planExecution(
        approved,
        { provider: 'google', kinds: ['email'] },
        { now: '2026-09-08T00:00:00.000Z' },
      ),
    ).toThrow(AppError);
    expect(() => planExecution(approved, null, { now })).toThrow(
      'Bu işlem için bağlı bir hesap gerekli.',
    );
    expect(() =>
      planExecution(approved, { provider: 'google', kinds: ['calendar'] }, { now }),
    ).toThrow(AppError);
    expect(() =>
      planExecution(approved, { provider: 'device', kinds: ['email'] }, { now }),
    ).toThrow('Bu hesap bu işlemi desteklemiyor.');
    const retry = transition(transition(approved, 'executing', { now }), 'failed', { now });
    expect(
      planExecution(
        transition(retry, 'executing', { now }),
        { provider: 'google', kinds: ['email'] },
        { now },
      ).kind,
    ).toBe('gmail_send');
