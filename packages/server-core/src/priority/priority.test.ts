...
import { applyFeedback, rankCandidates, scoreCandidate, selectTopPriorities, type PriorityCandidate, type PriorityContext } from './index';
...
function rule(type: PriorityRule['type'], value: string, position = 0): PriorityRule {
  return { id: `rule-${type}-${value}`, userId: 'u1', type, value, label: value, enabled: true, position, createdAt: now, updatedAt: now };
}

function vip(displayName: string, email: string | null, contactId: string | null = null): VipPerson {
  return { id: `vip-${displayName}`, userId: 'u1', displayName, email, contactId, relation: null, notifyAlways: true, createdAt: now, updatedAt: now };
}

function learned(kind: LearnedPreference['kind'], subjectKey: string, weight: number, statement = ''): LearnedPreference {
  return { id: `lp-${kind}-${subjectKey}`, userId: 'u1', kind, statement, subjectKey, weight, evidenceCount: 3, enabled: true, lastReinforcedAt: now, createdAt: now, updatedAt: now };
}

describe('priority · ordering', () => {
  it('mute rules produce muted:true, score 0 and a Turkish reason', () => {
    const r = scoreCandidate(cand({ id: 'a', senderEmail: 'bot@spam.io', importance: 'critical' }), ctx({ rules: [rule('mute_domain', 'spam.io')] }));
  ...
  it('explicit sender_important rule lifts a low AI importance to high with the canonical reason', () => {
    const r = scoreCandidate(cand({ id: 'a', senderEmail: 'ahmet@musteri.com', importance: 'low' }), ctx({ rules: [rule('sender_important', 'ahmet@musteri.com')] }));
  ...
  it('explicit rule beats learned preference in both directions', () => {
    ...
    const withoutRule = scoreCandidate(cand({ id: 'a', senderEmail: 'ahmet@musteri.com', importance: 'normal' }), ctx({ learned: [down] }));
    ...
    const lowRule = scoreCandidate(
      cand({ id: 'b', senderEmail: 'promo@shop.com', importance: 'high', text: 'Yeni kampanya başladı' }),
  ...
  it('explicit low rules even beat security and deadlines (user rules are law)', () => {
    const r = scoreCandidate(
      cand({ id: 'a', category: 'security', importance: 'critical', text: 'Haftalık güvenlik bülteni', deadlineAt: '2026-09-04T14:00:00.000Z' }),
      ctx({ rules: [rule('keyword_low', 'bülten')] }),
    );
    expect(r.tier).toBe('low');
  });
  it('security is critical/high regardless of promo flags', () => {
    const r = scoreCandidate(cand({ id: 'a', category: 'security', importance: 'critical', isNewsletter: true }), ctx());
    expect(r.tier).toBe('critical');
    expect(r.reasons).toContain('Güvenlik uyarısı');
    expect(scoreCandidate(cand({ id: 'b', category: 'security', importance: 'normal' }), ctx()).tier).toBe('high');
  });
  it('a deadline within 24h that needs a reply is critical with a natural reason', () => {
    const r = scoreCandidate(
      cand({ id: 'a', category: 'action_required', importance: 'normal', requiresUserAction: true, deadlineAt: '2026-09-04T14:00:00.000Z' }),
      ctx(),
    );
    expect(r.tier).toBe('critical');
  ...
  it('deadline tiers by horizon: 3 days → high, 6 days → normal floor, far → no floor, overdue → high', () => {
    const in3 = scoreCandidate(cand({ id: 'a', importance: 'low', deadlineAt: '2026-09-06T15:00:00.000Z', deadlineHasTime: false }), ctx());
    expect(in3.tier).toBe('high');
    expect(in3.reasons[0]).toBe('Son tarih: Pazar sonuna kadar');
    expect(scoreCandidate(cand({ id: 'b', importance: 'low', deadlineAt: '2026-09-10T15:00:00.000Z' }), ctx()).tier).toBe('normal');
    expect(scoreCandidate(cand({ id: 'c', importance: 'low', deadlineAt: '2026-10-10T15:00:00.000Z' }), ctx()).tier).toBe('low');
    const overdue = scoreCandidate(cand({ id: 'd', importance: 'low', deadlineAt: '2026-09-03T15:00:00.000Z' }), ctx());
  ...
  it('VIP person is at least high with "VIP: name"', () => {
    const r = scoreCandidate(cand({ id: 'a', senderEmail: 'ahmet@musteri.com', importance: 'low' }), ctx({ vips: [vip('Ahmet Yılmaz', 'ahmet@musteri.com')] }));
    expect(r.tier).toBe('high');
    expect(r.reasons).toContain('VIP: Ahmet Yılmaz');
    const byContact = scoreCandidate(cand({ id: 'b', contactId: 'c-1', importance: 'low' }), ctx({ vips: [vip('Elif', null, 'c-1')] }));
    expect(byContact.reasons).toContain('VIP: Elif');
  });
  it('waiting for reply, own commitment and meeting relevance set a normal floor', () => {
    expect(scoreCandidate(cand({ id: 'a', importance: 'low', category: 'waiting_for_user', requiresUserAction: true, ageHours: 60 }), ctx()).reasons).toContain('2 gündür senden yanıt bekleniyor');
    const c = scoreCandidate(cand({ id: 'b', kind: 'commitment', importance: 'low', isUserCommitment: true }), ctx());
    expect(c.tier).toBe('normal');
    expect(c.reasons).toContain('Kendi verdiğin bir söz');
    const m = scoreCandidate(cand({ id: 'c', importance: 'low', relatedMeetingAt: '2026-09-04T11:30:00.000Z' }), ctx());
  ...
  it('learned preference shifts the AI tier by one step and explains itself', () => {
    const up = scoreCandidate(cand({ id: 'a', contactId: 'c-9', importance: 'normal' }), ctx({ learned: [learned('person_priority', 'c-9', 0.7, 'Mehmet Yılmaz yüksek öncelikli.')] }));
    expect(up.tier).toBe('high');
    expect(up.reasons).toContain('Öğrendiğim: Mehmet Yılmaz yüksek öncelikli.');
    const down = scoreCandidate(cand({ id: 'b', category: 'information', importance: 'high' }), ctx({ learned: [learned('category_priority', 'information', -0.6)] }));
    expect(down.tier).toBe('normal');
    const weak = scoreCandidate(cand({ id: 'c', contactId: 'c-9', importance: 'normal' }), ctx({ learned: [learned('person_priority', 'c-9', 0.2)] }));
  ...
  it('AI importance decides the base tier; promotions drop to low when nothing above applies', () => {
    ...
    expect(scoreCandidate(cand({ id: 'b', importance: 'high' }), ctx()).reasons).toContain('Önemli görünüyor');
    ...
    const promoWithDeadline = scoreCandidate(cand({ id: 'd', importance: 'high', isPromotion: true, requiresUserAction: true, deadlineAt: '2026-09-04T14:00:00.000Z' }), ctx());
    expect(promoWithDeadline.tier).toBe('critical');
  });
  it('score is monotonic with tier and bounded 0..1000', () => {
    const c = scoreCandidate(cand({ id: 'a', importance: 'critical', category: 'security', requiresUserAction: true, deadlineAt: '2026-09-04T14:00:00.000Z' }), ctx({ vips: [vip('X', null)] }));
    const l = scoreCandidate(cand({ id: 'b', importance: 'low', isNewsletter: true, ageHours: 500 }), ctx());
  ...
  it('English locale variants', () => {
    const r = scoreCandidate(
      cand({ id: 'a', senderEmail: 'ahmet@musteri.com', requiresUserAction: true, deadlineAt: '2026-09-04T14:00:00.000Z' }),
...
describe('priority · ranking and selection', () => {
  it('rankCandidates is deterministic: tier, score, earliest deadline, freshness, id', () => {
    const items = [
      ...
      cand({ id: 'deadline-late', importance: 'normal', requiresUserAction: true, deadlineAt: '2026-09-05T14:00:00.000Z' }),
      cand({ id: 'deadline-soon', importance: 'normal', requiresUserAction: true, deadlineAt: '2026-09-04T14:00:00.000Z' }),
    ...
    expect(ranked.map((r) => r.candidate.id)).toEqual(['deadline-soon', 'security', 'deadline-late', 'plain', 'newsletter', 'muted']);
    const again = rankCandidates([...items].reverse(), ctx({ rules: [rule('mute_sender', 'x@muted.com')] }));
  ...
  it('selectTopPriorities keeps diversity across threads and people', () => {
    ...
    for (let i = 0; i < 6; i++) items.push(cand({ id: `ahmet-${i}`, importance: 'critical', senderEmail: 'ahmet@musteri.com', threadId: i < 3 ? 't-1' : `t-${i}`, ageHours: i }));
    items.push(cand({ id: 'elif', importance: 'high', senderEmail: 'elif@firma.com', threadId: 't-9' }));
    items.push(cand({ id: 'mehmet', importance: 'normal', senderEmail: 'mehmet@firma.com', threadId: 't-10' }));
    items.push(cand({ id: 'promo', importance: 'low', isPromotion: true, senderEmail: 'promo@shop.com' }));
  ...
  it('selectTopPriorities relaxes the person cap (never the thread cap) when nobody else is left', () => {
    const items: PriorityCandidate[] = [];
    for (let i = 0; i < 6; i++) items.push(cand({ id: `ahmet-${i}`, importance: 'critical', senderEmail: 'ahmet@musteri.com', threadId: i < 2 ? 't-1' : `t-${i}`, ageHours: i }));
    items.push(cand({ id: 'elif', importance: 'high', senderEmail: 'elif@firma.com', threadId: 't-9' }));
  ...
  it('selectTopPriorities never returns muted items and respects max', () => {
    const items = [cand({ id: 'a', senderEmail: 'x@muted.com' }), cand({ id: 'b' }), cand({ id: 'c' })];
    const top = selectTopPriorities(rankCandidates(items, ctx({ rules: [rule('mute_sender', 'x@muted.com')] })), { max: 2 });
    expect(top.map((r) => r.candidate.id)).toEqual(['b', 'c']);
  });
});

describe('priority · feedback', () => {
  const entity = { entityType: 'email_thread' as const, entityId: 'e-1', contactId: 'c-1', senderEmail: 'Ahmet@Musteri.com', senderName: 'Ahmet Yılmaz', category: 'information' as const };
  it('learns from not_important / important when learning is on', () => {
    ...
    expect(plan.learnedUpserts).toEqual([
      { kind: 'person_priority', subjectKey: 'c-1', weightDelta: -0.35, statement: 'Ahmet Yılmaz daha az öncelikli.' },
      { kind: 'category_priority', subjectKey: 'information', weightDelta: -0.15, statement: 'Bilgi kategorisi daha az önemli.' },
    ]);
  ...
  it('suggests explicit rules instead when learning is off', () => {
    ...
    expect(plan.ruleSuggestions[0]).toMatchObject({ type: 'sender_important', value: 'ahmet@musteri.com' });
    const promo = applyFeedback('not_important', { learnFromInteractions: false, entity: { ...entity, category: 'promotion' }, now });
  ...
  it('make_vip always produces a VipPerson upsert and a personal ack', () => {
    ...
    expect(plan.vipUpserts).toEqual([{ displayName: 'Ahmet Yılmaz', email: 'ahmet@musteri.com', contactId: 'c-1', notifyAlways: true }]);
    expect(plan.ack).toBe('Ahmet Yılmaz artık VIP.');
    const noPerson = applyFeedback('make_vip', { learnFromInteractions: true, entity: { entityType: 'life_event', entityId: 'le-1' }, now });
  ...
  it('stop_following closes the follow-up and records a dismiss pattern', () => {
    const plan = applyFeedback('stop_following', { learnFromInteractions: true, entity: { ...entity, entityType: 'follow_up', entityId: 'fu-7' }, now });
