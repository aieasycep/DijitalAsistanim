...
describe('followups · detection', () => {
  it('creates a nudge_due draft for a sent thread with no reply after the cadence', () => {
    const drafts = detectFollowUps({
      threads: [thread({ id: 't-mehmet', subject: 'Re: Teklif v2' })],
      now,
      userEmails,
  ...
  it('skips threads that do not deserve a follow-up', () => {
    const threads = [
      ...
      thread({
        subject: 'Kendime not',
        participants: [{ name: 'Yunus Emre', email: 'yunus@example.com' }],
      }),
      thread({
        subject: 'No-reply',
        participants: [{ email: 'yunus@example.com' }, { email: 'noreply@shop.com' }],
      }),
  ...
  it('respects existing follow-ups, dismissal limits and learned dismiss patterns', () => {
    ...
    const watching = detectFollowUps({
      threads: [t],
      now,
      userEmails,
      existing: [followUp({ threadId: 't-mehmet', status: 'watching' })],
    });
    expect(watching).toHaveLength(0);
    const closedAfter = detectFollowUps({
      threads: [t],
      now,
      userEmails,
      existing: [followUp({ threadId: 't-mehmet', status: 'closed', sentAt: t.lastMessageAt })],
    });
    expect(closedAfter).toHaveLength(0);
    const closedBefore = detectFollowUps({
      threads: [t],
      now,
      userEmails,
      existing: [
        followUp({ threadId: 't-mehmet', status: 'closed', sentAt: at('2026-08-20', '10:00') }),
      ],
    });
    ...
    const dismissed = detectFollowUps({
      ...
      contactsById: { 'c-mehmet': mehmetContact },
      existing: [
        followUp({ threadId: 't-old-1', contactId: 'c-mehmet', status: 'closed', dismissCount: 1 }),
        followUp({ threadId: 't-old-2', contactId: 'c-mehmet', status: 'closed', dismissCount: 1 }),
      ],
    });
    expect(dismissed).toHaveLength(0);
    const pattern = detectFollowUps({
      threads: [t],
      now,
      userEmails,
      learned: [learned('dismiss_pattern', 'mehmet@musteri.com', 0.5)],
    });
    expect(pattern).toHaveLength(0);
  });
  it('adjusts the cadence with learned follow_up_cadence and labels Outlook accounts', () => {
    const sooner = detectFollowUps({
      threads: [thread({ subject: 'A', lastMessageAt: at('2026-09-04', '10:00') })],
      now,
      userEmails,
      learned: [learned('follow_up_cadence', 'mehmet@musteri.com', 1)],
    });
    expect(sooner[0]?.nudgeAfterDays).toBe(2);
    const later = detectFollowUps({
      threads: [thread({ subject: 'B', lastMessageAt: at('2026-09-04', '10:00') })],
      now,
      userEmails,
      learned: [learned('follow_up_cadence', 'default', -1)],
    });
    ...
    const outlook = detectFollowUps({
      threads: [thread({ subject: 'C', accountId: 'acc-ms' })],
      now,
      userEmails,
      accountSourceTypes: { 'acc-ms': 'outlook' },
    });
...
describe('followups · lifecycle', () => {
  it('reply after the tracked message marks it replied; earlier replies are ignored', () => {
    ...
    expect(followUpStatusAfterReply(closeFollowUp(f, now), at('2026-09-06', '09:00')).status).toBe(
      'closed',
    );
  });
  it('snooze and close keep the record consistent', () => {
    ...
    expect(
      refreshFollowUpStatus(
        followUp({ status: 'watching', sentAt: at('2026-09-04', '10:00') }),
        now,
      ).status,
    ).toBe('watching');
  });
...
describe('followups · copy', () => {
  it('phrases the canonical Turkish card and English variant', () => {
    ...
    expect(followUpBrief(followUp({ topic: 'Sözleşme taslağı 4. madde' }), { now })).toBe(
      'Gönderdiğin “Sözleşme taslağı 4. madde” mailine 3 gündür cevap gelmedi.',
    );
    expect(
      followUpBrief(followUp({ topic: 'Teklif', sentAt: at('2026-09-05', '07:00') }), { now }),
    ).toBe('Bugün gönderdiğin teklif mailine henüz cevap gelmedi.');
    expect(followUpBrief(f, { now, locale: 'en' })).toBe(
      'No reply to your Teklif email for 3 days.',
    );
    expect(followUpWaitLabel(f, { now })).toBe('3 gün');
    expect(followUpWaitLabel(followUp({ sentAt: at('2026-09-05', '06:00') }), { now })).toBe(
      '2 saat',
    );
    expect(followUpWaitLabel(f, { now, locale: 'en' })).toBe('3 days');
    expect(followUpReason(f, { now })).toBe(
      'Son mesajı sen gönderdin ve Mehmet Yılmaz 3 gündür yanıt vermedi.',
    );
  });
});
  ...
  it('caps per day, one per thread and person, skips recently nudged and snoozed', () => {
    const candidates = [
      followUp({
        ...
        threadId: 't-a',
        counterpartName: 'Mehmet Yılmaz',
        sentAt: at('2026-08-30', '10:00'),
      }),
      followUp({
        id: 'b',
        threadId: 't-b',
        counterpartName: 'Selin Kaya',
        sentAt: at('2026-08-31', '10:00'),
      }),
      followUp({
        id: 'c',
        threadId: 't-c',
        counterpartName: 'Ahmet Yılmaz',
        sentAt: at('2026-09-01', '10:00'),
      }),
      followUp({
        id: 'd',
        threadId: 't-d',
        counterpartName: 'Ayşe Demir',
        sentAt: at('2026-09-01', '12:00'),
      }),
      followUp({
        id: 'same-person',
        threadId: 't-e',
        counterpartName: 'Mehmet Yılmaz',
        sentAt: at('2026-08-29', '10:00'),
      }),
      followUp({
        id: 'recent',
        threadId: 't-recent',
        counterpartName: 'Ali',
        sentAt: at('2026-08-20', '10:00'),
      }),
      followUp({
        id: 'snoozed',
        threadId: 't-s',
        counterpartName: 'Veli',
        status: 'snoozed',
        snoozedUntil: at('2026-09-08', '09:00'),
      }),
      followUp({
        id: 'watching',
        threadId: 't-w',
    ...
    const base = {
      ...
      timezone: tz,
      maxPerDay: 3,
      minHoursBetweenSameThread: 48,
      lastNudgeAtByThread: { 't-recent': at('2026-09-04', '09:00') },
    };
    const picked = selectNudges(candidates, base);
    expect(picked.map((f) => f.id)).toEqual(['same-person', 'b', 'c']);
    const budget = selectNudges(candidates, {
      ...base,
      sentToday: [at('2026-09-05', '07:00'), at('2026-09-05', '07:30')],
    });
    expect(budget.map((f) => f.id)).toEqual(['same-person']);
    expect(
      selectNudges(candidates, {
        ...base,
        sentToday: [
          at('2026-09-05', '07:00'),
          at('2026-09-05', '07:30'),
    ...
    const yesterdayDoesNotCount = selectNudges(candidates, {
      ...base,
      maxPerDay: 1,
      sentToday: [at('2026-09-04', '07:00')],
    });
