...
import type { CalendarEvent, Commitment, FollowUp, Insight, LifeEvent, TaskItem, WeeklyMetrics } from '@da/domain';
...
let seq = 0;
function insight(partial: Partial<Insight> & { title: string; kind: Insight['kind']; badge: Insight['badge']; entityType: Insight['entityType']; entityId: string }): Insight {
  seq += 1;
  return {
    ...
    status: 'active',
    snoozedUntil: null,
    source: { type: 'gmail', id: partial.entityId, label: 'Gmail', timestamp: at('2026-09-05', '06:00') },
    actions: [],
    tags: ['mail'],
    forDate: '2026-09-05',
    confidence: 0.9,
...
const ahmet = insight({
  ...
  source: { type: 'gmail', id: 'e1', label: 'Gmail', person: 'Ahmet Yılmaz', timestamp: at('2026-09-05', '08:42') },
...
const meeting = insight({
  ...
  source: { type: 'google_calendar', id: 'd1', label: 'Google Takvim', person: 'Mehmet Yılmaz', personId: 'c-mehmet', timestamp: at('2026-09-05', '14:30'), url: 'https://meet.google.com/abc' },
...
const application = insight({
  ...
  timeLabel: '17:00',
  dueAt: at('2026-09-05', '17:00'),
  source: { type: 'gmail', id: 'e3', label: 'Gmail', person: 'Girişim Programı', timestamp: at('2026-09-04', '16:10') },
  tags: ['important', 'mail'],
});
...
const selin = insight({
  ...
  source: { type: 'gmail', id: 'e2', label: 'Gmail', person: 'Selin Kaya', timestamp: at('2026-09-04', '15:40') },
  actions: [{ id: 'reply', label: 'Yanıtla', kind: 'reply', primary: true }],
  tags: ['important', 'mail'],
});
const cargo = insight({
  ...
  source: { type: 'gmail', id: 's1', label: 'Kargo', person: 'Yurtiçi', timestamp: at('2026-09-04', '19:02') },
  tags: ['personal'],
});
const promiseInsight = insight({
  ...
  source: { type: 'meeting_note', id: 'n1', label: 'Toplantı notu', person: 'Mehmet Yılmaz', personId: 'c-mehmet', timestamp: at('2026-09-01', '15:31') },
  tags: ['follow_up'],
});
const newSinceMorning = insight({
  ...
  priorityScore: 900,
  timeLabel: '12:10',
  source: { type: 'gmail', id: 'e7', label: 'Gmail', person: 'Ahmet Yılmaz', timestamp: at('2026-09-05', '12:10') },
...
const events = [
  event({ id: 'd1', title: 'Mehmet ile müşteri toplantısı', startAt: at('2026-09-05', '14:30'), endAt: at('2026-09-05', '15:30'), location: 'Ofis', attendees: [{ name: 'Mehmet Yılmaz', email: 'mehmet@musteri.com', isOrganizer: false, responseStatus: 'accepted' }] }),
  event({ id: 'd2', title: 'Ürün gözden geçirme', startAt: at('2026-09-05', '16:00'), endAt: at('2026-09-05', '16:30'), meetingUrl: 'https://teams.microsoft.com/x', location: 'Online' }),
  event({ id: 'd3', title: 'Akşam yemeği · Karaköy', startAt: at('2026-09-05', '20:30'), endAt: at('2026-09-05', '22:30'), location: 'Karaköy, İstanbul', source: 'apple_calendar' }),
  event({ id: 'd4', title: 'Haftalık ekip', startAt: at('2026-09-06', '09:00'), endAt: at('2026-09-06', '10:00'), location: 'Ofis' }),
  event({ id: 'd5', title: 'Doktor randevusu', startAt: at('2026-09-07', '14:30'), endAt: at('2026-09-07', '15:15'), location: 'Nişantaşı' }),
  event({ id: 'dx', title: 'İptal', startAt: at('2026-09-05', '11:00'), endAt: at('2026-09-05', '12:00'), status: 'cancelled' }),
];
const followUps: FollowUp[] = [
  {
    ...
    repliedAt: null,
    closedAt: null,
    source: { type: 'gmail', id: 'e4', label: 'Gmail', person: 'Mehmet Yılmaz', timestamp: at('2026-09-02', '10:15') },
    dismissCount: 0,
    createdAt: now,
    updatedAt: now,
  },
...
const commitments: Commitment[] = [
  { id: 'g3', userId: 'u1', text: 'Mehmet Teklif v2 geri bildirimi gönderecek', direction: 'other_owes', counterpartName: 'Mehmet Yılmaz', dueAt: at('2026-09-05', '18:00'), dueText: 'bu hafta', status: 'open', source: { type: 'gmail', id: 'e4', label: 'Gmail', person: 'Mehmet Yılmaz', timestamp: at('2026-09-02', '10:15') }, confidence: 0.7, createdAt: now, updatedAt: now },
];
const lifeEvents: LifeEvent[] = [
  { id: 's1', userId: 'u1', type: 'shipment', title: 'Trendyol siparişin bugün geliyor.', details: { carrier: 'Yurtiçi Kargo', deliveryWindow: { start: at('2026-09-05', '14:00'), end: at('2026-09-05', '18:00') } }, eventAt: at('2026-09-05', '14:00'), status: 'today', source: { type: 'gmail', id: 'e5', label: 'Kargo', timestamp: at('2026-09-04', '19:02') }, confidence: 0.93, dedupeKey: 'l1', createdAt: now, updatedAt: now },
  { id: 's2', userId: 'u1', type: 'flight', title: 'TK2412 İstanbul → Antalya', details: { airline: 'THY', departureAt: at('2026-09-06', '09:15'), checkInUrl: 'https://thy.example' }, eventAt: at('2026-09-06', '09:15'), status: 'upcoming', source: { type: 'gmail', id: 'e6', label: 'THY', timestamp: at('2026-08-28', '11:20') }, confidence: 0.95, dedupeKey: 'l2', createdAt: now, updatedAt: now },
  { id: 's3', userId: 'u1', type: 'payment', title: 'Elektrik faturası · 1.842 TL', details: { payee: 'CK Enerji', amount: 1842, currency: 'TRY', dueAt: at('2026-09-10', '23:59') }, eventAt: at('2026-09-10', '23:59'), status: 'upcoming', source: { type: 'gmail', id: 'e7', label: 'Gmail', person: 'CK Enerji', timestamp: at('2026-09-03', '09:05') }, confidence: 0.9, dedupeKey: 'l3', createdAt: now, updatedAt: now },
  { id: 's4', userId: 'u1', type: 'subscription', title: 'Netflix yenilenecek', details: { serviceName: 'Netflix', renewsAt: at('2026-09-09', '00:00') }, eventAt: at('2026-09-09', '00:00'), status: 'upcoming', source: { type: 'gmail', id: 'e8', label: 'Gmail', person: 'Netflix', timestamp: at('2026-09-02', '07:30') }, confidence: 0.9, dedupeKey: 'l4', createdAt: now, updatedAt: now },
];

function ctx(partial: Partial<BriefingContext> = {}): BriefingContext {
  return { insights, events, followUps, commitments, lifeEvents, now, timezone: tz, userName: 'Yunus', counts: { analyzedEmails: 46, analyzedCalendars: 1, analyzedDays: 3 }, ...partial };
}
...
describe('briefing · candidates', () => {
  it('morning: six fixed sections in order, real items only, empty sections hidden', () => {
    ...
    expect(c.sections.map((s) => s.section)).toEqual(['priorities', 'schedule', 'waiting_for_you', 'waiting_for_others', 'deadlines', 'personal']);
    const priorities = c.sections[0]?.items ?? [];
    expect(priorities).toHaveLength(5);
    expect(priorities[0]).toMatchObject({ icon: 'mail', title: "Ahmet senden bugün 17:00'ye kadar revize teklif bekliyor.", meta: 'Acil · 08:42', insightId: 'i-ahmet', entityType: 'email_thread', entityId: 'e1', candidateId: 'priorities:email_thread:e1', status: 'open' });
    expect(priorities.filter((p) => p.source?.personId === 'c-mehmet').length).toBeLessThanOrEqual(2);
    ...
    const empty = assembleBriefingCandidates('morning', ctx({ insights: [], events: [], followUps: [], commitments: [], lifeEvents: [] }));
    expect(empty.sections).toEqual([]);
  });
  it('midday: only changes since the morning and the rest of the day', () => {
    const c = assembleBriefingCandidates('midday', ctx({ insights: [...insights, newSinceMorning], now: at('2026-09-05', '13:00'), sinceAt: at('2026-09-05', '08:00') }));
    ...
    const quiet = assembleBriefingCandidates('midday', ctx({ now: at('2026-09-05', '13:00'), sinceAt: at('2026-09-05', '08:00') }));
  ...
  it('evening: completed, carried over, follow-ups and tomorrow’s first event', () => {
    const doneTask: TaskItem = { id: 't9', userId: 'u1', title: 'Rapor gönder', dueAt: null, status: 'completed', completedAt: at('2026-09-05', '15:10'), source: null, provider: 'internal', scheduledStartAt: null, scheduledEndAt: null, priority: 'normal', createdAt: now, updatedAt: now };
    const c = assembleBriefingCandidates('evening', ctx({ now: at('2026-09-05', '19:00'), completedToday: [{ ...ahmet, status: 'completed', updatedAt: at('2026-09-05', '16:40') }], tasksDoneToday: [doneTask], insights: [selin, application, promiseInsight, meeting, cargo] }));
    expect(c.sections.map((s) => s.section)).toEqual(['completed', 'carried_over', 'follow_ups', 'first_event_tomorrow']);
    ...
    expect(c.sections[3]?.items[0]).toMatchObject({ title: 'Haftalık ekip', meta: 'Yarın 09:00 · 60 dk · Ofis', entityId: 'd4' });
  });
  it('weekly: metrics-driven sections', () => {
    const c = assembleBriefingCandidates('weekly', ctx({ weekly }));
    expect(c.sections.map((s) => s.section)).toEqual(['priorities', 'deadlines', 'follow_ups', 'schedule']);
    expect(c.sections[3]?.items.map((i) => i.meta)).toEqual(['Bugün 14:30 · 60 dk · Ofis', 'Bugün 16:00 · 30 dk · Online', 'Bugün 20:30 · 120 dk · Karaköy, İstanbul', 'Yarın 09:00 · 60 dk · Ofis', '7 Eylül 14:30 · 45 dk · Nişantaşı']);
  });
  it('produces prompt candidates with stable ids', () => {
    const c = assembleBriefingCandidates('morning', ctx());
    const prompt = toBriefingPromptCandidates(c, { insights });
    expect(prompt[0]).toMatchObject({ id: 'priorities:email_thread:e1', section: 'priorities', importance: 'critical', source: 'Gmail · Ahmet Yılmaz' });
...
describe('briefing · fallback composition', () => {
  it('morning: headline, subline, mood and a grounded Lora narrative with audio chapters', () => {
    ...
    expect(chapters.map((ch) => ch.title)).toEqual(['Genel bakış', 'Bugünün öncelikleri', 'Programın', 'Cevap bekleyenler', 'Son tarihler', 'Kişisel gelişmeler']);
    expect(chapters[0]?.text.startsWith('Günaydın Yunus.')).toBe(true);
    expect(chapters[2]?.text).toBe("Saat 14:30'da Mehmet ile müşteri toplantısı, saat 16:00'da Ürün gözden geçirme, saat 20:30'da Akşam yemeği, Karaköy.");
  ...
  it('morning with nothing to report', () => {
    const empty = ctx({ insights: [], events: [], followUps: [], commitments: [], lifeEvents: [], counts: { analyzedEmails: 12, analyzedCalendars: 1, analyzedDays: 1 } });
    const b = composeBriefingFallback('morning', assembleBriefingCandidates('morning', empty), empty);
    expect(b.headline).toBe('Bugün her şey kontrol altında.');
    expect(b.highlightNumber).toBe(0);
    expect(b.narrative).toBe('Bugün takvimin oldukça sakin. Gelen 12 mail arasında dikkat gerektiren bir konu yok.');
    expect(b.audio?.chapters).toHaveLength(1);
  });
  it('midday and evening headlines', () => {
    const mid = ctx({ insights: [...insights, newSinceMorning], now: at('2026-09-05', '13:00'), sinceAt: at('2026-09-05', '08:00') });
    ...
    expect(m.narrative).toContain("Günün kalanında 3 etkinliğin var; ilki saat 14:30'da Mehmet ile müşteri toplantısı.");
    const quietCtx = ctx({ now: at('2026-09-05', '13:00'), sinceAt: at('2026-09-05', '08:00') });
    const q = composeBriefingFallback('midday', assembleBriefingCandidates('midday', quietCtx), quietCtx);
    expect(q.headline).toBe('Her şey planlandığı gibi.');
    expect(q.hasChanges).toBe(false);
    expect(q.highlightNumber).toBe(0);

    const eveCtx = ctx({ now: at('2026-09-05', '19:00'), completedToday: [{ ...ahmet, status: 'completed' }], insights: [selin, application, promiseInsight] });
    const e = composeBriefingFallback('evening', assembleBriefingCandidates('evening', eveCtx), eveCtx);
    expect(e.headline).toBe('Bugünden yarına 3 konu kaldı.');
    expect(e.subline).toBe('1 tamamlandı · 2 takip · Yarın 09:00 Haftalık ekip');
    expect(e.narrative).toBe("Bugün 1 konuyu tamamladın. Yarına 3 konu kaldı. Selin sözleşme taslağı için yorumunu bekliyor. Mehmet Yılmaz 3 gündür yanıt vermedi. Yarın ilk etkinliğin saat 09:00'da Haftalık ekip.");
  ...
  it('weekly narrative and outlook come from metrics only', () => {
    ...
    expect(w.narrative).toBe('Bu hafta 312 mail analiz edildi, 18 önemli konu öne çıkarıldı. 9 toplantının 7 tanesi için hazırlık notu hazırdı. 6 takibin 4 tanesi cevaplandı. 5 son tarihin hiçbiri kaçmadı. Yaklaşık 2 sa 15 dk kazandın.');
  ...
  it('speaks English when asked', () => {
    ...
    const b = composeBriefingFallback('morning', assembleBriefingCandidates('morning', enCtx), enCtx);
...
describe('briefing · AI merge, carry-over and share card', () => {
  ...
  const ai: BriefingAi = {
    ...
    outlook: null,
    sections: [
      { section: 'priorities', itemIds: ['priorities:calendar_event:d1', 'nope:invented:1', 'priorities:email_thread:e1'] },
  ...
  it('accepts narration, keeps counts and subline, reorders only known ids', () => {
    ...
    expect(priorities.map((i) => i.candidateId).slice(0, 2)).toEqual(['priorities:calendar_event:d1', 'priorities:email_thread:e1']);
    expect(priorities).toHaveLength(5);
    expect(priorities.map((i) => i.position)).toEqual([0, 1, 2, 3, 4]);
    expect(merged.items.filter((i) => i.section === 'schedule').map((i) => i.candidateId)).toEqual(fallback.items.filter((i) => i.section === 'schedule').map((i) => i.candidateId));
  ...
  it('rejects a headline whose number contradicts the highlight number and keeps fallback audio when the script is empty', () => {
    const merged = mergeAiBriefing(fallback, { ...ai, headline: 'Bugün bilmen gereken 9 şey var.', audioScript: [] }, known);
  ...
  it('plans the evening carry-over from selected insight ids only', () => {
    const eveCtx = ctx({ now: at('2026-09-05', '19:00'), insights: [selin, application, promiseInsight] });
    const eve = composeBriefingFallback('evening', assembleBriefingCandidates('evening', eveCtx), eveCtx);
    const plan = eveningCarryOverPlan(eve, ['i-selin', 'i-promise', 'i-unknown', 'i-selin'], { tomorrowDateKey: '2026-09-06', now: at('2026-09-05', '19:05') });
  ...
  it('share text carries metrics only, never names or subjects', () => {
    ...
    expect(text).toBe(['DİJİTAL HAFTAM · 7–13 Eylül', '312 mail analiz edildi', '18 önemli konu öne çıkarıldı', '9 toplantı · 7 hazırlık notu', '6 takip · 4 cevaplandı', '5 son tarih, hiçbiri kaçmadı', 'Kazandığın zaman: 2 sa 15 dk', 'Dijital Asistan · dijitalasistan.app'].join('\n'));
    expect(text).not.toContain('Mehmet');
    expect(text).not.toContain('Çarşamba');
    expect(weeklyShareText({ ...weekly, weekStart: '2026-09-28', weekEnd: '2026-10-04', deadlinesMissed: 1 })).toContain('28 Eylül – 4 Ekim');
    expect(weeklyShareText({ ...weekly, deadlinesMissed: 1 })).toContain('5 son tarih, 1 tanesi kaçtı');
  });
});
