...
export type LifeEventLike = LifeEventExtraction &
  Partial<
    Pick<ExtractedLifeEvent, 'evidence' | 'occurredAt' | 'provider' | 'delivered' | 'billKind'>
  >;
...
function whenLabel(
  iso: string,
  locale: Locale,
  opts: LifeEventTitleOptions,
  withTime = false,
): string {
  const timezone = timezoneOf(opts);
  if (opts.now && !Number.isNaN(Date.parse(opts.now)))
    return formatDateLocative(iso, { now: opts.now, timezone, locale, withTime });
  const d = localDateOf(iso, timezone);
  const time = withTime ? ` ${clock(iso, timezone)}` : '';
  if (locale === 'tr')
    return `${turkishLocative(`${d.d} ${MONTHS_TR_TITLE[d.m - 1] ?? ''}`)}${time}`;
...
function dayTimeLabel(
  ...
  const timezone = timezoneOf(opts);
  if (opts.now && !Number.isNaN(Date.parse(opts.now)))
    return formatDateLabel(iso, { now: opts.now, timezone, locale, withTime });
  const d = localDateOf(iso, timezone);
...
function securityTitle(
  ...
  const tr = (withService: string, without: string): string =>
    s ? `${s} ${withService}` : without;
  const en = (withService: string, without: string): string =>
    s ? withService.replace('{s}', s) : without;
  switch (event) {
    case 'Şüpheli giriş':
      return locale === 'tr'
        ? tr('hesabında şüpheli giriş.', 'Hesabında şüpheli giriş.')
        : en('Suspicious sign-in to your {s} account.', 'Suspicious sign-in to your account.');
    case 'Şifre değişikliği':
      return locale === 'tr'
        ? tr('şifren değiştirildi.', 'Şifren değiştirildi.')
        : en('Your {s} password was changed.', 'Your password was changed.');
    case 'Şifre sıfırlandı':
      return locale === 'tr'
        ? tr('şifren sıfırlandı.', 'Şifren sıfırlandı.')
        : en('Your {s} password was reset.', 'Your password was reset.');
    case 'İki adımlı doğrulama':
      return locale === 'tr'
        ? tr('iki adımlı doğrulama ayarın değişti.', 'İki adımlı doğrulama ayarın değişti.')
        : en('{s} two-step verification changed.', 'Two-step verification changed.');
    case 'Kurtarma bilgisi değişti':
      return locale === 'tr'
        ? tr('kurtarma bilgin değişti.', 'Kurtarma bilgin değişti.')
        : en('{s} recovery info changed.', 'Recovery info changed.');
    case 'Yeni cihazdan giriş':
    case 'Yeni giriş':
      return locale === 'tr'
        ? tr('hesabında yeni giriş.', 'Hesabında yeni giriş.')
        : en('New sign-in to your {s} account.', 'New sign-in to your account.');
    default:
      return locale === 'tr'
        ? tr('güvenlik uyarısı.', 'Güvenlik uyarısı.')
        : en('{s} security alert.', 'Security alert.');
  }
}

/** Natural one-line title: "Trendyol siparişin bugün geliyor.", "TK2412 · İstanbul → Antalya", "Elektrik faturası · 1.842 TL". */
export function lifeEventTitle(
  extraction: LifeEventLike,
  locale: Locale = 'tr',
  opts: LifeEventTitleOptions = {},
): string {
  ...
  switch (extraction.type) {
    case 'shipment': {
      const subject = tr
        ? d.merchant
          ? `${d.merchant} siparişin`
          : 'Kargon'
        : d.merchant
          ? `Your ${d.merchant} order`
          : 'Your parcel';
      if (extraction.delivered)
        title = tr ? `${subject} teslim edildi.` : `${subject} was delivered.`;
      else if (d.deliveryWindow?.start)
        title = tr
          ? `${subject} ${whenLabel(d.deliveryWindow.start, locale, opts)} geliyor.`
          : `${subject} arrives ${whenLabel(d.deliveryWindow.start, locale, opts)}.`;
    ...
    case 'flight': {
      ...
      if (d.from && d.to) title = `${label} · ${cityOf(d.from)} → ${cityOf(d.to)}`;
        ...
        title = `${label} ${tr ? 'uçuşu' : 'flight'} · ${dayTimeLabel(d.departureAt, locale, opts, true)}`;
      else title = `${label} ${tr ? 'uçuşu' : 'flight'}`;
      break;
    }
    case 'reservation': {
      const head = d.venue
        ? `${d.venue} ${tr ? 'rezervasyonu' : 'reservation'}`
        : tr
          ? 'Rezervasyon'
          : 'Reservation';
      title = d.reservationAt
        ? `${head} · ${dayTimeLabel(d.reservationAt, locale, opts, true)}`
        : head;
      break;
    }
    case 'payment': {
      const kind = extraction.billKind
        ? BILL_LABELS[locale][extraction.billKind]
        : d.payee
          ? tr
            ? `${d.payee} faturası`
            : `${d.payee} bill`
          : tr
            ? 'Fatura'
            : 'Bill';
      if (typeof d.amount === 'number' && d.currency)
        title = `${kind} · ${formatAmount(d.amount, d.currency, locale)}`;
      else if (d.dueAt)
        title = tr
          ? `${kind} · son ödeme ${dayTimeLabel(d.dueAt, locale, opts, false)}`
          : `${kind} · due ${dayTimeLabel(d.dueAt, locale, opts, false)}`;
      else title = kind;
      break;
    }
    case 'subscription': {
      ...
      if (d.renewsAt)
        title = tr
          ? `${service} ${whenLabel(d.renewsAt, locale, opts)} yenileniyor.`
          : `${service} renews ${whenLabel(d.renewsAt, locale, opts)}.`;
      else
        title = tr ? `${service} aboneliğin yenileniyor.` : `Your ${service} subscription renews.`;
      break;
    }
    case 'security':
...
export function lifeEventEventAt(extraction: LifeEventLike): string | null {
  ...
  switch (extraction.type) {
    case 'shipment':
      return (
        d.deliveryWindow?.start ?? (extraction.delivered ? (extraction.occurredAt ?? null) : null)
      );
    case 'flight':
...
const ACTION_LABELS: Record<
  Locale,
  Record<
    'track' | 'check_in' | 'add_to_calendar' | 'open_link' | 'remind' | 'open_original',
    string
  >
> = {
  tr: {
    ...
    open_link: 'Faturayı Aç',
    remind: 'Hatırlat',
    open_original: 'Kaynağı Aç',
  },
  en: {
    track: 'Track',
    check_in: 'Check-in',
 ...
 */
export function lifeEventActions(
  extraction: LifeEventLike,
  locale: Locale = 'tr',
): SuggestedAction[] {
  const d = extraction.details;
  ...
  switch (extraction.type) {
    case 'shipment':
      if (d.trackingUrl)
        out.push({ kind: 'track', label: labels.track, payload: { url: d.trackingUrl } });
      break;
    case 'flight':
      if (d.checkInUrl)
        out.push({ kind: 'check_in', label: labels.check_in, payload: { url: d.checkInUrl } });
      if (d.departureAt) {
        out.push({
          kind: 'add_to_calendar',
          label: labels.add_to_calendar,
          payload: {
            title: lifeEventTitle(extraction, locale),
            startAt: d.departureAt,
            ...(d.arrivalAt ? { endAt: d.arrivalAt } : {}),
          },
        });
      }
      break;
    case 'reservation':
      if (d.reservationAt)
        out.push({
          kind: 'add_to_calendar',
          label: labels.add_to_calendar,
          payload: { title: lifeEventTitle(extraction, locale), startAt: d.reservationAt },
        });
      break;
    case 'payment':
      if (d.paymentUrl)
        out.push({ kind: 'open_link', label: labels.open_link, payload: { url: d.paymentUrl } });
      if (d.dueAt) out.push({ kind: 'remind', label: labels.remind, payload: { at: d.dueAt } });
      break;
    case 'subscription':
      if (d.renewsAt)
        out.push({ kind: 'remind', label: labels.remind, payload: { at: d.renewsAt } });
      break;
    case 'security':
...
export function lifeEventStatus(
  extraction: LifeEventLike,
  now: string,
  timezone: string,
): LifeEventStatusValue {
  ...
  if (!at || Number.isNaN(Date.parse(at)))
    return extraction.type === 'security' ? 'today' : 'upcoming';
...
export function lifeEventDedupeKey(
  ...
  const day = (iso: string | null | undefined): string =>
    iso && !Number.isNaN(Date.parse(iso)) ? localDateKey(iso, timezone) : '';
  const d = extraction.details;
  ...
  switch (extraction.type) {
    case 'shipment':
      if (d.trackingNumber) parts.push(d.trackingNumber);
        ...
        parts.push(
          slug(d.merchant ?? d.carrier ?? extraction.provider ?? 'kargo'),
          day(d.deliveryWindow?.start),
        );
      break;
    case 'flight':
      parts.push(
        d.flightNumber ?? slug(d.airline ?? extraction.provider ?? 'ucus'),
        day(d.departureAt),
      );
      break;
    case 'reservation':
    ...
    case 'payment':
      parts.push(
        slug(d.payee ?? extraction.provider ?? 'odeme'),
        day(d.dueAt) || (typeof d.amount === 'number' ? String(d.amount) : ''),
      );
      break;
    case 'subscription':
    ...
    case 'security':
      parts.push(
        slug(d.securityEvent ?? 'guvenlik'),
        slug(extraction.provider ?? ''),
        day(extraction.occurredAt),
      );
      break;
  }
