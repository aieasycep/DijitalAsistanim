...
const RE_RENEWAL =
  /(?<![\p{L}])(?:yenilenecek(?:tir)?|yenileniyor|yenilenir|yenileme|otomatik olarak yenilen\p{L}*|otomatik yenileme|renews?|renewal|will renew|auto-?renews?|auto-?renewal|will be charged|charged|faturalandırılacak|ücretlendirilecek|tahsil edilecek|deneme süre\p{L}*|trial (?:ends|period|expires)|aboneliğiniz|üyeliğiniz|subscription|membership)(?![\p{L}])/u;
const RE_RENEW_DATE =
  /(?<![\p{L}])(?:yenilen|yenileme|renew|charged|faturalandır|ücretlendir|tahsil|deneme|trial|sona er|expires|bitiyor|biter)/u;
const RE_AMOUNT_LABEL =
  /(?:ücret|tutar|fiyat|price|amount|charged|cost|bedel|aylık|yıllık|monthly|yearly|annual|plan)/u;

function findService(ctx: Ctx): string | null {
  const inSender = ctx.senderOrg
    ? brandInText(ctx.senderOrg.toLocaleLowerCase('tr-TR'), SERVICES)
    : null;
...
export function detectSubscription(ctx: Ctx): ExtractedLifeEvent | null {
  ...
  const renews =
    dateNear(ctx, RE_RENEW_DATE, (d) => d.kind !== 'time') ??
    deadlineFromText({ text: ctx.text, now: ctx.now, timezone: ctx.timezone }) ??
