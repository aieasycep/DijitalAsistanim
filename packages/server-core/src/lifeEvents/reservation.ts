...
const RE_RESERVATION = /(?<![\p{L}])(?:rezervasyon(?:unuz|un|u|)|reservation|booking|masa(?:nız|n)?|check-?in tarihi|giriş tarihi|konaklama(?:nız)?|otel|hotel|randevu(?:nuz)?|bilet(?:iniz|leriniz)?|tickets?|etkinlik|event|konser|concert|table for|your table)(?![\p{L}])/u;
const RE_CONFIRMED = /(?<![\p{L}])(?:onaylandı|onaylanmıştır|confirmed|confirmation|oluşturuldu|alındı|is booked|has been booked|booked|rezerve edildi|ayrıldı)(?![\p{L}])/u;
const RE_VENUE = /(?<venue>\p{Lu}[\p{L}&'’.-]+(?:\s+\p{Lu}[\p{L}&'’.-]+){0,3}\s+(?:Restaurant|Restoran|Restorant|Otel|Oteli|Hotel|Resort|Cafe|Café|Kafe|Bistro|Lokanta|Lokantası|Meyhane|Meyhanesi|Steakhouse|Brasserie|Balık|Ocakbaşı|Kebap|Pizzeria|Bar|Lounge|Club|Kulübü|Sahne|Arena|Stadyum|Stadı|Salonu|Tiyatrosu|Theatre|Theater|Hall|Merkezi|Center|Centre|Spa|Kliniği|Clinic|Hastanesi|Hospital))(?![\p{L}])/u;
const RE_PARTY = /(?<![\d])(?<n>\d{1,3})\s*(?:kişilik|kişi|guests?|people|persons?|pax|misafir|yetişkin|adults?)(?![\p{L}])/iu;
const RE_DATE_LABEL = /(?<![\p{L}])(?:tarih|date|check-?in|giriş|saat|time|rezervasyon|reservation|booking|masa|table|randevu|etkinlik|event|konser|başlangıç|start)(?![\p{L}])/u;
...
export function detectReservation(ctx: Ctx): ExtractedLifeEvent | null {
  ...
  const venueLabel = labelledValue(ctx.text, /mekan|mekân|venue|otel|hotel|restoran|restaurant|yer|konum|location|place/);
  ...
  if (venue) {
    ...
    else if (venueMatch) ctx.evidence.add(venueMatch.index, venueMatch.index + venueMatch[0].length);
    confidence += 0.1;
  } else if (ctx.senderOrg && !/(?<![\p{L}])(?:booking\.com|airbnb|obilet|biletix|passo|opentable|resy|thefork|zomato|yemeksepeti|getir)(?![\p{L}])/iu.test(ctx.senderOrg)) {
  ...
  const when = dateNear(ctx, RE_DATE_LABEL, (d) => d.kind !== 'time' && d.hasTime) ?? dateNear(ctx, RE_DATE_LABEL, (d) => d.kind !== 'time') ?? ctx.dates.find((d) => d.kind !== 'time') ?? null;
  if (when) {
