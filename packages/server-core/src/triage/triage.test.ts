...
type MailFixture = Omit<Partial<TriageEmailInput>, 'from' | 'subject'> & {
  from: string;
  subject: string;
};
...
describe('triage · stage 1 provider labels and sender shape', () => {
  it('spam and trash are skipped', () => {
    expect(
      triageEmail(mail({ from: 'x@spam.com', subject: 'Kazandınız!', labels: ['SPAM'] }), ctxBase)
        .bucket,
    ).toBe('skip');
    expect(
      triageEmail(mail({ from: 'x@firma.com', subject: 'Eski', labels: ['TRASH'] }), ctxBase)
        .bucket,
    ).toBe('skip');
  });
  it('promotions with subject heuristics land in low with a Turkish reason', () => {
    const r = triageEmail(
      mail({
        from: 'kampanya@e.trendyol.com',
        subject: 'Sana özel %50 indirim! Kampanya bugün son',
  ...
  it('newsletters (List-Unsubscribe / Precedence: bulk) are low even from a human-looking address', () => {
    const a = triageEmail(
      mail({
        from: 'ayse@haberbulteni.com',
        subject: 'Haftalık bülten #42',
        listUnsubscribe: '<mailto:unsub@haberbulteni.com>',
      }),
      ctxBase,
    );
    ...
    const b = triageEmail(
      mail({ from: 'team@product.io', subject: 'What is new this week', precedence: 'bulk' }),
      { ...ctxBase, locale: 'en' },
    );
  ...
  it('social/updates labels are low unless a rule-level signal hits', () => {
    const social = triageEmail(
      mail({
        from: 'notification@facebookmail.com',
        subject: 'Ahmet fotoğrafını beğendi',
        labels: ['CATEGORY_SOCIAL'],
      }),
      ctxBase,
    );
    expect(social.bucket).toBe('low');
    const updates = triageEmail(
      mail({
        from: 'no-reply@service.com',
        subject: 'Haftalık özetiniz hazır',
    ...
    const shipment = triageEmail(
      mail({
        from: 'bilgi@yurticikargo.com',
        subject: 'Kargonuz dağıtıma çıktı',
        snippet: 'Takip numarası: 1234567890123',
        labels: ['CATEGORY_UPDATES'],
      }),
      ctxBase,
    );
  ...
  it('known automated senders are recognised, security senders are not skipped', () => {
    ...
    const r = triageEmail(
      mail({
        ...
        snippet: 'Google Hesabınıza yeni bir cihazdan giriş yapıldı: Chrome, Windows, İstanbul.',
      }),
      ctxBase,
    );
  ...
  it('out-of-office auto replies are skipped', () => {
    const r = triageEmail(
      mail({
        from: 'mehmet@firma.com',
        subject: 'Otomatik yanıt: Teklif',
...
describe('triage · stage 2 rules and signals', () => {
  it('mute_sender / mute_domain rules skip the mail, but security mail still passes', () => {
    const rules = [
      rule('mute_domain', 'spamly.io', 'spamly.io'),
      rule('mute_sender', 'bot@firma.com', 'bot'),
    ];
    expect(
      triageEmail(mail({ from: 'news@spamly.io', subject: 'Merhaba' }), { ...ctxBase, rules })
        .bucket,
    ).toBe('skip');
    const muted = triageEmail(mail({ from: 'bot@firma.com', subject: 'Rapor' }), {
      ...ctxBase,
      rules,
    });
    expect(muted.bucket).toBe('skip');
    expect(muted.reasons[0]).toBe('Kuralın: bot sessize alınmış');
    const sec = triageEmail(mail({ from: 'bot@firma.com', subject: 'Şifreniz değiştirildi' }), {
      ...ctxBase,
      rules,
    });
  ...
  it('VIP sender goes to ai with high importance and a VIP reason', () => {
    const r = triageEmail(mail({ from: 'ahmet@musteri.com', subject: 'Teklif hakkında' }), {
      ...ctxBase,
      vips: [vip('Ahmet Yılmaz', 'ahmet@musteri.com')],
    });
  ...
  it('keyword_low and promotions_low rules push mail to low', () => {
    const r = triageEmail(mail({ from: 'hr@firma.com', subject: 'Haftalık yemek listesi' }), {
      ...ctxBase,
      rules: [rule('keyword_low', 'yemek listesi', 'Yemek listesi')],
    });
    expect(r.bucket).toBe('low');
  ...
  it('meeting terms are detected (Turkish and English)', () => {
    expect(
      triageEmail(
        mail({ from: 'mehmet@firma.com', subject: 'Yarınki toplantı için gündem' }),
        ctxBase,
      ).preCategory,
    ).toBe('meeting');
    expect(
      triageEmail(mail({ from: 'lisa@partner.com', subject: 'Zoom invite: Q4 planning' }), ctxBase)
        .signals.meeting,
    ).toBe(true);
    const cal = triageEmail(
      mail({
        from: 'calendar-notification@google.com',
        subject: 'Davet: Müşteri toplantısı @ Per 10 Eyl 2026 14:00',
      }),
      ctxBase,
    );
  ...
  it('finance, travel and shipment notifications from automated senders take the rules bucket', () => {
    const invoice = triageEmail(
      mail({
        from: 'bildirim@ckenerji.com.tr',
        subject: 'Eylül ayı elektrik faturanız',
        snippet: 'Fatura tutarı 1.842,50 TL. Son ödeme tarihi 10 Eylül 2026.',
        sentAt: now,
      }),
      ctxBase,
    );
    ...
    const ship = triageEmail(
      mail({
        from: 'info@trendyolmail.com',
        subject: 'Siparişin kargoya verildi',
        snippet: 'Yurtiçi Kargo takip numarası 987654321',
      }),
      ctxBase,
    );
  ...
  it('a plain human email goes to ai with the generic reason', () => {
    const r = triageEmail(
      mail({ from: 'elif@firma.com', subject: 'Selam', snippet: 'Bugün öğlen yemeğe çıkalım mı?' }),
      ctxBase,
    );
    expect(r.bucket).toBe('ai');
  ...
  it('mail sent by the user is handled by rules (follow-up watching), asking mails become waiting_for_other', () => {
    const r = triageEmail(
      mail({
        from: 'me@firma.com',
        subject: 'Teklif',
        snippet: 'Ekte teklif var, görüşünüzü bekliyorum.',
        isFromUser: true,
      }),
      ctxBase,
    );
    ...
    const info = triageEmail(
      mail({
        from: 'me@firma.com',
        subject: 'Bilgi',
        snippet: 'Tamam, not aldım.',
        isFromUser: true,
      }),
      ctxBase,
    );
  ...
  it('OTP mails take the security fast path with low importance; password change is critical', () => {
    const otp = triageEmail(
      mail({
        from: 'noreply@bank.com.tr',
        subject: 'Doğrulama kodunuz',
        snippet: 'Kodunuz: 482193. Kimseyle paylaşmayın.',
      }),
      ctxBase,
    );
  ...
  it('a weak "şifre" mention from a human does not trigger the fast path', () => {
    const r = triageEmail(
      mail({
        from: 'elif@firma.com',
        subject: 'Wifi şifresi',
        snippet: 'Ofis wifi şifresi değişti, yarın söylerim.',
      }),
      ctxBase,
    );
    expect(r.bucket).toBe('ai');
...
describe('triage · Android notification privacy guard', () => {
  it('excludes authenticator and password manager packages', () => {
    ...
    ).toEqual({ sensitive: true, reason: 'excluded_package' });
    expect(
      isSensitiveNotification({
    ...
    ).toBe('excluded_package');
    expect(
      isSensitiveNotification({
        packageName: 'com.microsoft.authenticator',
        title: 'Approve sign-in?',
        text: '',
      }).sensitive,
    ).toBe(true);
    expect(
      isSensitiveNotification({ packageName: 'org.example.myauthenticator', title: 'x', text: 'y' })
        .reason,
    ).toBe('excluded_package');
  });
  it('detects OTP codes in Turkish and English SMS-style notifications', () => {
    expect(
      isSensitiveNotification({
        ...
        title: 'BANKAM',
        text: 'Doğrulama kodunuz: 482193. Kimseyle paylaşmayın.',
      }),
    ).toEqual({ sensitive: true, reason: 'otp' });
    expect(
      isSensitiveNotification({
  ...
  it('does not flag order/tracking numbers or ordinary messages', () => {
    expect(
      isSensitiveNotification({
        ...
        title: 'Trendyol',
        text: 'Sipariş kodun 48213 kargoya verildi',
      }).sensitive,
    ).toBe(false);
    expect(
      isSensitiveNotification({
        packageName: 'com.whatsapp',
        title: 'Ahmet',
        text: 'Yarın 14:00 uygun mu?',
      }).sensitive,
    ).toBe(false);
    expect(
      isSensitiveNotification({
        packageName: 'com.yurticikargo',
        title: 'Yurtiçi',
        text: 'Takip no 1234567890 dağıtımda',
      }).sensitive,
    ).toBe(false);
  });
  it('flags credential disclosures and honours custom package lists', () => {
    expect(
      isSensitiveNotification({
        ...
        title: 'Hesap',
        text: 'Şifreniz: abc123',
      }).reason,
    ).toBe('credential');
    expect(
      isSensitiveNotification(
        { packageName: 'com.mybank', title: 'x', text: 'y' },
        { excludedPackages: ['com.mybank'] },
      ).reason,
    ).toBe('excluded_package');
  });
});
