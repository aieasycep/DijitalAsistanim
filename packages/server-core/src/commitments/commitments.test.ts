...
function run(
  text: string,
  partial: Partial<ExtractCommitmentsInput> & { authorIsUser: boolean; name?: string | null },
): CommitmentCandidate[] {
  ...
  return extractCommitments({
    ...
    now,
    timezone: tz,
    counterpartHint: name ? { name } : null,
    ...rest,
  });
}
...
describe('commitments · the user promises (Turkish)', () => {
  it('future 1sg with a dative counterpart and a relative due date', () => {
    const c = only("Mehmet'e yarın teklif göndereceğim.", {
      authorIsUser: true,
      name: 'Mehmet Yılmaz',
    });
  ...
  it('deadline cue "10 Eylül\'e kadar" and the hint counterpart in the verb\'s case', () => {
    const c = only("Kontrol edip 10 Eylül'e kadar size döneceğim.", {
      authorIsUser: true,
      name: 'Ahmet',
    });
  ...
  it('splits "ve" clauses when both carry a verb form; "bu akşam" keeps its clock time', () => {
    const r = run("Bu akşam sözleşmeyi imzalayacağım ve Pazartesi Mehmet'e ileteceğim.", {
      authorIsUser: true,
    });
  ...
  it('polite hedges do not cancel a commitment; intensifiers raise confidence', () => {
    ...
    const strong = only('Kesinlikle yarına kadar göndereceğim, söz.', {
      authorIsUser: true,
      name: 'Ahmet',
    });
...
describe('commitments · the other party promises or asks', () => {
  it('seed reply "hafta içinde dönüş yapacağım" → other_owes due this Friday', () => {
    const c = only('Teşekkürler, hafta içinde dönüş yapacağım.', {
      authorIsUser: false,
      name: 'Mehmet Yılmaz',
    });
  ...
  it('request forms: rica ederim / -rseniz sevinirim / lütfen + imperative', () => {
    expect(
      only('Göndermenizi rica ederim.', { authorIsUser: false, name: 'Ahmet Yılmaz' }).text,
    ).toBe("Ahmet'e gönder");
    const cond = only('Belgeleri gönderirseniz sevinirim.', {
      authorIsUser: false,
      name: 'Ahmet Yılmaz',
    });
  ...
  it('"cevabınızı bekliyorum" from the counterpart → the user owes a reply', () => {
    const c = only('Cevabınızı en kısa sürede bekliyorum.', {
      authorIsUser: false,
      name: 'Ahmet Yılmaz',
    });
  ...
  it("the counterpart's plans count only when directed at the user, dated or about a deliverable", () => {
    expect(
      run('Yönetim toplantısında sunacağım.', { authorIsUser: false, name: 'Ahmet Yılmaz' }),
    ).toHaveLength(0);
    expect(only('Sizi arayacağım.', { authorIsUser: false, name: 'Ahmet Yılmaz' }).text).toBe(
      'Ahmet sizi arayacak',
    );
    expect(
      only('Toplantıdan sonra size döneceğim.', { authorIsUser: false, name: 'Mehmet Yılmaz' })
        .text,
    ).toBe('Mehmet size dönecek');
    expect(only('Konuya bakacağım.', { authorIsUser: false, name: 'Ahmet Yılmaz' }).text).toBe(
      'Ahmet konuya bakacak',
    );
  });
});
...
describe('commitments · false positives', () => {
  ...
  it('conditionals', () => {
    ...
    expect(
      run('If you send the invoice, I will pay it tomorrow.', { authorIsUser: true }),
    ).toHaveLength(0);
  });
...
describe('commitments · English', () => {
  it('"I\'ll send you the proposal tomorrow" binds "you" to the hint', () => {
    const c = only("I'll send you the proposal tomorrow.", {
      authorIsUser: true,
      name: 'Mehmet Yılmaz',
      locale: 'en',
    });
  ...
  it('"and" clauses inherit the subject: "We\'ll review the draft and let you know by Friday."', () => {
    const r = run("We'll review the draft and let you know by Friday.", {
      authorIsUser: false,
      name: 'John Smith',
      locale: 'en',
    });
  ...
  it('"waiting for your reply" and "can you share …" requests', () => {
    const wait = only('I am waiting for your reply.', {
      authorIsUser: false,
      name: 'John',
      locale: 'en',
    });
    ...
    const share = only('Can you share the file by Friday?', {
      authorIsUser: true,
      name: 'Selin',
      locale: 'en',
    });
...
describe('commitments · helpers', () => {
  it('normalizeCommitmentText composes the imperative from a verbatim quote', () => {
    expect(
      normalizeCommitmentText('yarın göndereceğim', 'Mehmet Yılmaz', 'tr', { topic: 'Teklif v2' }),
    ).toBe("Mehmet'e teklif v2 gönder");
    expect(
      normalizeCommitmentText('hafta içinde dönüş yapacağım', 'Mehmet Yılmaz', 'tr', {
        direction: 'other_owes',
      }),
    ).toBe('Mehmet dönüş yapacak');
    expect(normalizeCommitmentText("Selin'i yarın ararım.", null, 'tr')).toBe("Selin'i ara");
    expect(normalizeCommitmentText("I'll send it tomorrow", 'Mehmet Yılmaz', 'en')).toBe(
      'Send it to Mehmet',
    );
  ...
  it('toCommitmentDraft maps the candidate onto the Commitment row shape', () => {
    ...
    const weak = only('Belgeleri gönderirseniz sevinirim.', {
      authorIsUser: false,
      name: 'Ahmet Yılmaz',
    });
