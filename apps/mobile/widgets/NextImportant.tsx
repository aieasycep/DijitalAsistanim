 ...
 */
export function NextImportantWidget(
  props: NextImportantProps,
      ...
      : tone === 'warning'
        ...
        : tone === 'accent'
          ...
          : tone === 'success'
            ? c.success
            : c.secondary;
  const toneSoft = (tone: string): string =>
    tone === 'critical'
      ? c.criticalSoft
      : tone === 'warning'
        ? c.warningSoft
    ...
    return (
      <HStack spacing={4} modifiers={[widgetURL(props.todayUrl)]}>
        ...
        <Text modifiers={[font({ size: 12, weight: 'medium' }), lineLimit(1)]}>
          {props.signedIn ? props.inlineLabel : props.signedOutTitle}
        </Text>
      </HStack>
    );
  }
    ...
    return (
      <ZStack modifiers={[widgetURL(props.todayUrl)]}>
        ...
        <VStack spacing={1} alignment="center">
          <Text modifiers={[font({ size: 9, weight: 'semibold' }), kerning(0.5), opacity(0.8)]}>
            {props.circularLabel}
          </Text>
          <Text modifiers={[font({ size: 20, weight: 'semibold' })]}>
            {props.signedIn ? String(props.count) : '–'}
          </Text>
        </VStack>
      </ZStack>
    );
      ...
      <ZStack modifiers={[widgetURL(rectUrl)]}>
        ...
        <HStack>
          ...
          >
            ...
            >
              {rect ? rect.kicker : props.kicker}
            </Text>
            <Text modifiers={[font({ size: 13, weight: 'semibold' }), lineLimit(2)]}>
              {!props.signedIn ? props.signedOutTitle : rect ? rect.title : props.emptyTitle}
            </Text>
            {rect?.sub ? (
              <Text modifiers={[font({ size: 10 }), opacity(0.8), lineLimit(1)]}>{rect.sub}</Text>
            ) : null}
          </VStack>
          <Spacer />
        </HStack>
  ...
  return (
    <VStack
      alignment="leading"
      spacing={0}
      modifiers={[containerBackground(c.bg, 'widget'), widgetURL(url), padding({ all: 14 })]}
    >
      <HStack spacing={6}>
        ...
        {props.signedIn && props.item?.badgeLabel ? (
          <Text
            modifiers={[
              ...
              kerning(0.5),
              foregroundStyle(toneColor(props.item.tone)),
              padding({ horizontal: 6, vertical: 2 }),
              background(toneSoft(props.item.tone)),
              clipShape('capsule'),
            ]}
          >
      ...
      {!props.signedIn ? (
        ...
        <VStack alignment="leading" spacing={6}>
          ...
          <Text modifiers={[font({ size: 11 }), foregroundStyle(c.tertiary), lineLimit(1)]}>
            {props.item.meta}
          </Text>
        </VStack>
      ) : (
