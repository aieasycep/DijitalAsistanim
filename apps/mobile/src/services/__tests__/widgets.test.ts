...
jest.mock('@/lib/env', () => ({
  env: { appScheme: 'dijitalasistan', appVersion: '1.0.0', isProduction: false, universalHosts: ['dijitalasistan.app'], webUrl: 'https://dijitalasistan.app' },
  IS_PRODUCTION: false,
  hasSupabase: false,
  isDemoMode: true,
}));
...
const mockHandles: Record<string, { updateTimeline: jest.Mock; updateSnapshot: jest.Mock; reload: jest.Mock }> = {};
...
jest.mock('@expo/ui/swift-ui', () => ({ AccessoryWidgetBackground: jest.fn(), HStack: jest.fn(), Image: jest.fn(), Link: jest.fn(), Spacer: jest.fn(), Text: jest.fn(), VStack: jest.fn(), ZStack: jest.fn() }));
...
function insight(overrides: Partial<Insight>): Insight {
  ...
  return {
    ...
    status: 'active',
    snoozedUntil: null,
    source: { type: 'gmail', id: `msg-${seq}`, label: 'Gmail', person: 'Ahmet', timestamp: '2030-09-05T05:42:00Z' },
    actions: [],
...
const briefing: Briefing = {
  ...
  counts: { importantEmails: 3, events: 4, followUps: 2, deadlines: 1, total: 5, analyzedEmails: 127, analyzedCalendars: 2, analyzedDays: 3 },
  items: [],
...
function fullFeed(): TodayFeed {
  const p1 = insight({ id: 'p1', title: "Ahmet 17:00'ye kadar revize teklif bekliyor.", badge: 'urgent', timeLabel: '17:00', entityId: 'thread-ahmet' });
  const p2 = insight({ id: 'p2', title: 'Mehmet ile müşteri toplantısı', badge: 'meeting', kind: 'meeting', timeLabel: '14:30', entityType: 'calendar_event', entityId: 'evt-mehmet', dueAt: '2030-09-05T11:30:00Z' });
  const p3 = insight({ id: 'p3', title: 'Başvuru kapanıyor', badge: 'deadline', kind: 'deadline', timeLabel: '17:00' });
  const p4 = insight({ id: 'p4', title: 'Selin · Teklif v2', badge: 'follow_up', kind: 'follow_up', subtitle: '3 gündür yanıt yok', entityType: 'follow_up', entityId: 'fu-1' });
  const done = insight({ id: 'p5', title: 'Tamamlanan iş', status: 'completed' });
  const m1 = insight({ id: 'm1', title: 'Mehmet ile müşteri toplantısı', kind: 'meeting', badge: 'meeting', entityType: 'calendar_event', entityId: 'evt-mehmet', dueAt: '2030-09-05T11:30:00Z', subtitle: 'Hazırlık hazır · 3 konu' });
  const m2 = insight({ id: 'm2', title: 'Ekip standup', kind: 'meeting', badge: 'calendar', entityType: 'calendar_event', entityId: 'evt-standup', dueAt: '2030-09-05T13:00:00Z' });
  const past = insight({ id: 'm0', title: 'Sabah kahvesi', kind: 'meeting', badge: 'calendar', entityType: 'calendar_event', entityId: 'evt-past', dueAt: '2030-09-05T05:00:00Z' });
  return feed({ briefing, priorities: [p1, p2, p3, p4, done], meetings: [m2, past, m1], pendingApprovals: 2 });
}
...
describe('buildWidgetSnapshot', () => {
  ...
  it('builds priorities, next event, follow-up and briefing data from a full feed', () => {
    ...
    expect(s.priorities[0]).toEqual({ id: 'p1', title: "Ahmet 17:00'ye kadar revize teklif bekliyor.", timeLabel: '17:00', badge: 'urgent', deepLink: 'dijitalasistan://email/thread-ahmet' });
    ...
    expect(s.nextEvent).toEqual({ id: 'evt-mehmet', title: 'Mehmet ile müşteri toplantısı', time: '14:30', startAt: '2030-09-05T11:30:00Z', deepLink: 'dijitalasistan://meeting/evt-mehmet/prep', sub: 'Hazırlık hazır · 3 konu' });
    expect(s.openFollowUps).toBe(2);
    expect(s.followUp).toEqual({ title: 'Selin · Teklif v2', sub: '3 gündür yanıt yok', deepLink: 'dijitalasistan://followups' });
  ...
  it('exposes counts only when lock-screen privacy is generic', () => {
    ...
    expect(s.priorities.every((p) => !p.title.includes('Ahmet') && !p.title.includes('Mehmet'))).toBe(true);
...
describe('props & timeline', () => {
  it('resolves every label app-side for the three widgets', () => {
    ...
    expect(next.item).toEqual({ title: "Ahmet 17:00'ye kadar revize teklif bekliyor.", badgeLabel: 'ACİL', tone: 'critical', meta: '17:00', deepLink: 'dijitalasistan://email/thread-ahmet' });
    expect(next.count).toBe(5);
    expect(next.inlineLabel).toBe('5 önemli konu · 17:00');
    expect(next.circularLabel).toBe('ÖNEMLİ');
    expect(next.rectangular).toEqual({ kicker: 'SIRADAKİ · 14:30', title: 'Mehmet ile müşteri toplantısı', sub: 'Hazırlık hazır · 3 konu', deepLink: 'dijitalasistan://meeting/evt-mehmet/prep' });
    ...
    expect(brief.nextEvent).toEqual({ hour: '14', minute: '30', title: 'Mehmet ile müşteri toplantısı', sub: 'Hazırlık hazır · 3 konu', deepLink: 'dijitalasistan://meeting/evt-mehmet/prep' });
  ...
  it('maps insight entities to contract deep links', () => {
    expect(deepLinkForInsight({ entityType: 'email_thread', entityId: 'x' })).toBe('dijitalasistan://email/x');
    expect(deepLinkForInsight({ entityType: 'calendar_event', entityId: 'x' })).toBe('dijitalasistan://meeting/x/prep');
    expect(deepLinkForInsight({ entityType: 'conflict', entityId: 'x' })).toBe('dijitalasistan://conflict/x');
    expect(deepLinkForInsight({ entityType: 'life_event', entityId: 'x' })).toBe('dijitalasistan://life/x');
    expect(deepLinkForInsight({ entityType: 'suggestion', entityId: 'x' })).toBe('dijitalasistan://today');
  });
});
...
describe('sync', () => {
  it('persists the snapshot and pushes a timeline to every widget', async () => {
    ...
    for (const name of ['NextImportant', 'TodayPriorities', 'DailyBrief']) {
      ...
      const entries = handle?.updateTimeline.mock.calls[0]?.[0] as { date: Date; props: { signedIn: boolean } }[];
  ...
  it('pushes the signed-out snapshot', async () => {
    ...
    expect(s.signedIn).toBe(false);
    const entries = mockHandles.DailyBrief?.updateTimeline.mock.calls[0]?.[0] as { props: { signedIn: boolean; signedOutTitle: string } }[];
    expect(entries[0]?.props).toMatchObject({ signedIn: false, signedOutTitle: 'Giriş yapınca burada özet görünür.' });
  });
});
