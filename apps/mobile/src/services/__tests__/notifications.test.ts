...
jest.mock('expo-application', () => ({
  ...
  getIosIdForVendorAsync: async () => 'ios-vendor-id',
}));
...
describe('applyLockScreenPrivacy', () => {
  it('keeps everything in full mode', () => {
    expect(applyLockScreenPrivacy('Ahmet revize teklif bekliyor', 'Detay', 'full')).toEqual({
      title: 'Ahmet revize teklif bekliyor',
      body: 'Detay',
    });
  });
  it('strips the body in title_only mode', () => {
    expect(applyLockScreenPrivacy('Ahmet revize teklif bekliyor', 'Detay', 'title_only')).toEqual({
      title: 'Ahmet revize teklif bekliyor',
      body: null,
    });
  });
...
describe('quiet hours', () => {
  ...
  it('handles same-day ranges, disabled and degenerate settings', () => {
    ...
    expect(
      isWithinQuietHours(prefs({ quietHoursEnabled: false }), new Date('2030-09-05T20:30:00Z'), TZ),
    ).toBe(false);
    expect(
      isWithinQuietHours(
        prefs({ quietHoursStart: '08:00', quietHoursEnd: '08:00' }),
        new Date('2030-09-05T20:30:00Z'),
        TZ,
      ),
    ).toBe(false);
  ...
  it('computes the end of quiet hours', () => {
    expect(quietHoursEndAfter(new Date('2030-09-05T20:30:00Z'), p, TZ).toISOString()).toBe(
      '2030-09-06T05:00:00.000Z',
    );
    expect(quietHoursEndAfter(new Date('2030-09-05T04:30:00Z'), p, TZ).toISOString()).toBe(
      '2030-09-05T05:00:00.000Z',
    );
  });
  it('applies the delivery policy', () => {
    const at = new Date('2030-09-05T20:30:00Z');
    expect(resolveQuietHours(at, p, 'defer', TZ)).toEqual({
      at: new Date('2030-09-06T05:00:00.000Z'),
      silent: false,
    });
...
describe('foreground behavior & channels', () => {
  ...
  it('suppresses disabled categories entirely', () => {
    expect(
      decideForegroundBehavior(
        { category: 'follow_up' },
        prefs(),
        new Date('2030-09-05T09:00:00Z'),
        TZ,
      ),
    ).toMatchObject({ shouldShowBanner: false, shouldShowList: false, shouldPlaySound: false });
  });
  it('mutes sound during quiet hours and sets the badge for approvals', () => {
    ...
    ).toMatchObject({ shouldShowBanner: true, shouldPlaySound: false, shouldSetBadge: false });
    expect(
      decideForegroundBehavior(
        { category: 'critical_email' },
        prefs(),
    ...
    ).toMatchObject({ shouldSetBadge: true, shouldShowList: true });
    expect(decideForegroundBehavior({ category: 'morning' }, null, new Date(), TZ)).toMatchObject({
  ...
  it('reads payload metadata defensively', () => {
    ...
    expect(readNotificationMeta({ category: 'bogus', url: 42 })).toEqual({
      category: null,
      deepLink: null,
      insightKind: null,
      badge: null,
...
describe('permission & push token', () => {
  ...
  it('registers once per user/token and skips unchanged registrations', async () => {
    ...
    expect(await registerPushToken(ds, { userId: 'u1' })).toEqual({
      status: 'registered',
    ...
    expect(await registerPushToken(ds, { userId: 'u2' })).toEqual({
      status: 'registered',
      token: 'ExponentPushToken[abc]',
    });
...
describe('local scheduling', () => {
  it('defers reminders out of quiet hours and strips the body in title_only mode', async () => {
    ...
    const id = await scheduleLocalReminder({
      ...
      body: 'Ahmet bekliyor',
      at: '2030-09-05T20:30:00Z',
      deepLink: 'dijitalasistan://commitments',
    });
    ...
    expect(req?.trigger).toEqual({
      type: 'date',
      date: new Date('2030-09-06T05:00:00.000Z'),
      channelId: 'reminders',
    });
  ...
  it('keeps meeting leads on time but silent during quiet hours, and dedupes', async () => {
    ...
    expect(mockScheduled[0]?.trigger).toEqual({
      type: 'date',
      date: new Date('2030-09-05T04:00:00.000Z'),
      channelId: 'meetings',
    });
    expect(await scheduleMeetingLeadNotification(event, 30)).toBe('meeting:evt-1');
    expect(mockScheduled).toHaveLength(1);
