jest.mock('expo-constants', () => ({
  ...
  default: {
    expoConfig: {
      extra: {
        universalHosts: ['dijitalasistan.app', 'www.dijitalasistan.app'],
...
describe('parseDeepLink — scheme URLs', () => {
  ...
  it('validates the flow filter and plan date', () => {
    expect(parseDeepLink('dijitalasistan://flow?filter=mail')).toEqual({
      href: '/(tabs)/flow',
      params: { filter: 'mail' },
    });
    expect(parseDeepLink('dijitalasistan://flow?filter=spam')).toEqual({ href: '/(tabs)/flow' });
    expect(parseDeepLink('dijitalasistan://plan?date=2026-09-05')).toEqual({
      href: '/(tabs)/plan',
  ...
  it('decodes free-text queries', () => {
    expect(parseDeepLink('dijitalasistan://assistant?q=Bug%C3%BCn%20ne%20var')).toEqual({
      href: '/(tabs)/assistant',
      params: { q: 'Bugün ne var' },
    });
    expect(parseDeepLink('dijitalasistan://search?q=teklif+v2')).toEqual({
      href: '/search',
      params: { q: 'teklif v2' },
    });
  });
  ...
  it('routes briefings by kind, audio requires an id', () => {
    expect(parseDeepLink(`dijitalasistan://briefing/morning?id=${ID}&autoplay=1`)).toEqual({
      href: '/briefing/morning',
      params: { id: ID, autoplay: '1' },
    });
    expect(parseDeepLink('dijitalasistan://briefing/evening')).toEqual({
      href: '/briefing/evening',
    });
    expect(parseDeepLink(`dijitalasistan://briefing/audio?id=${ID}`)).toEqual({
      href: '/briefing/audio',
      params: { id: ID },
    });
  ...
  it('routes email detail and reply, rejects other sub-paths and malformed ids', () => {
    expect(parseDeepLink(`dijitalasistan://email/${ID}`)).toEqual({
      href: `/email/${ID}`,
      params: { id: ID },
    });
    expect(parseDeepLink(`dijitalasistan://email/${ID}/reply`)).toEqual({
      href: `/email/${ID}/reply`,
      params: { id: ID },
    });
    expect(parseDeepLink('dijitalasistan://email/thread-demo-3')).toEqual({
      href: '/email/thread-demo-3',
      params: { id: 'thread-demo-3' },
    });
  ...
  it('routes meeting prep / post only', () => {
    expect(parseDeepLink(`dijitalasistan://meeting/${ID}/prep`)).toEqual({
      href: `/meeting/${ID}/prep`,
      params: { id: ID },
    });
    expect(parseDeepLink(`dijitalasistan://meeting/${ID}/post`)).toEqual({
      href: `/meeting/${ID}/post`,
      params: { id: ID },
    });
  ...
  it('routes entity screens and lists', () => {
    expect(parseDeepLink(`dijitalasistan://conflict/${ID}`)).toEqual({
      href: `/conflict/${ID}`,
      params: { id: ID },
    });
    expect(parseDeepLink(`dijitalasistan://person/${ID}`)).toEqual({
      href: `/person/${ID}`,
      params: { id: ID },
    });
    expect(parseDeepLink(`dijitalasistan://life/${ID}`)).toEqual({
      href: `/life/${ID}`,
      params: { id: ID },
    });
    expect(parseDeepLink('dijitalasistan://approvals')).toEqual({ href: '/approvals' });
    expect(parseDeepLink(`dijitalasistan://approvals/${ID}`)).toEqual({
      href: `/approvals/${ID}`,
      params: { id: ID },
    });
  ...
  it('routes capture, settings, paywall and referral with validated params', () => {
    expect(parseDeepLink(`dijitalasistan://capture?id=${ID}`)).toEqual({
      href: '/capture',
      params: { id: ID },
    });
    ...
    expect(parseDeepLink('dijitalasistan://settings/notifications')).toEqual({
      href: '/settings/notifications',
    });
    expect(parseDeepLink('dijitalasistan://settings/secret')).toBeNull();
    expect(parseDeepLink('dijitalasistan://paywall?context=meeting_prep')).toEqual({
      href: '/paywall',
      params: { context: 'meeting_prep' },
    });
    expect(parseDeepLink('dijitalasistan://paywall?context=<script>')).toEqual({
      href: '/paywall',
    });
    expect(parseDeepLink('dijitalasistan://referral?code=YUNUS-14')).toEqual({
      href: '/referral',
      params: { code: 'YUNUS-14' },
    });
  });

  it('routes OAuth returns with state/status/accountId/error', () => {
    expect(
      parseDeepLink(`dijitalasistan://oauth/google?state=abc&status=ok&accountId=${ID}`),
    ).toEqual({
      href: '/oauth/google',
  ...
  it('returns the auth callback pseudo-route with query and fragment merged', () => {
    const parsed = parseDeepLink(
      'dijitalasistan://auth/callback?code=pkce-code#access_token=at&refresh_token=rt',
    );
  ...
  it('rejects share-extension payloads, dev-client launch URLs and unknown paths', () => {
    ...
    expect(
      parseDeepLink('dijitalasistan://expo-development-client/?url=http%3A%2F%2F10.0.0.2%3A8081'),
    ).toBeNull();
...
describe('parseDeepLink — universal links and Expo dev URLs', () => {
  it('accepts allow-listed hosts under /app', () => {
    expect(parseDeepLink(`https://dijitalasistan.app/app/email/${ID}`)).toEqual({
      href: `/email/${ID}`,
      params: { id: ID },
    });
    expect(parseDeepLink('https://www.dijitalasistan.app/app')).toEqual({ href: '/(tabs)/today' });
    expect(parseDeepLink('https://dijitalasistan.app/app/flow?filter=calendar')).toEqual({
      href: '/(tabs)/flow',
  ...
  it('handles exp:// URLs with the /--/ separator', () => {
    expect(parseDeepLink(`exp://192.168.1.4:8081/--/email/${ID}`)).toEqual({
      href: `/email/${ID}`,
      params: { id: ID },
    });
    ...
    expect(parseDeepLink('dijitalasistan://192.168.1.4:8081/--/approvals')).toEqual({
      href: '/approvals',
    });
  });
});
...
describe('helpers', () => {
  it('parseQueryString keeps the first value, decodes + and %xx, skips broken escapes', () => {
    expect(parseQueryString('a=1&a=2&b=x+y&c=%E2%9C%93&bad=%E0%A4%A&empty=')).toEqual({
      a: '1',
      b: 'x y',
      c: '✓',
      empty: '',
