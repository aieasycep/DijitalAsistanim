...
jest.mock('expo-linking', () => ({ canOpenURL: jest.fn(async () => false), openURL: jest.fn(async () => true), openSettings: jest.fn() }));
...
jest.mock('expo-calendar/legacy', () => ({
  ...
  getCalendarPermissionsAsync: jest.fn(async () => ({ status: 'granted', granted: true, canAskAgain: true, expires: 'never' })),
  requestCalendarPermissionsAsync: jest.fn(async () => ({ status: 'denied', granted: false, canAskAgain: false, expires: 'never' })),
  getCalendarsAsync: jest.fn(async () => [
    { id: 'cal-1', title: 'İş', color: '#5B5CE2', source: { name: 'iCloud', type: 'caldav' }, isPrimary: true, allowsModifications: true, type: 'caldav' },
    { id: 'cal-bd', title: 'Doğum günleri', color: null, source: { name: 'Other', type: 'birthdays' }, isPrimary: false, allowsModifications: false, type: 'birthdays' },
  ]),
...
import { createDeviceEvent, listDeviceCalendars, mapDeviceEvent, requestCalendarPermission, syncDeviceCalendar } from '@/services/calendarBridge';
...
const baseEvent: Event = {
  ...
  organizer: { isCurrentUser: false, name: 'Mehmet', role: 'chair', status: 'accepted', type: 'person' },
...
describe('mapDeviceEvent', () => {
  ...
  it('uses the platform source, cancelled status and instance ids for recurrences', () => {
    const mapped = mapDeviceEvent({ ...baseEvent, status: 'canceled' as Event['status'], instanceId: 'evt-1#2', organizer: undefined, organizerEmail: undefined, location: null, notes: '' }, 'acc-1', 'android');
    expect(mapped).toMatchObject({ source: 'device_calendar', status: 'cancelled', externalEventId: 'evt-1#2', organizerIsUser: true, attendees: [], meetingUrl: null, meetingProvider: null, location: null, description: null });
  });
...
describe('device calendar bridge', () => {
  ...
  it('lists calendars without birthday calendars', async () => {
    expect(await listDeviceCalendars()).toEqual([{ id: 'cal-1', title: 'İş', color: '#5B5CE2', sourceName: 'iCloud', isPrimary: true, allowsModifications: true }]);
  });
  ...
  it('uploads mapped events in chunks', async () => {
    ...
    const result = await syncDeviceCalendar(ds, 'acc-1', ['cal-1'], { now: new Date('2030-09-05T06:00:00Z'), pastDays: 1, futureDays: 1 });
    expect(result).toEqual({ uploaded: 250 });
    expect(upsertDeviceEvents).toHaveBeenCalledTimes(2);
    expect(Calendar.getEventsAsync).toHaveBeenCalledWith(['cal-1'], new Date('2030-09-04T06:00:00Z'), new Date('2030-09-06T06:00:00Z'));
    expect(await syncDeviceCalendar(ds, 'acc-1', [])).toEqual({ uploaded: 0 });
  });

  it('writes approved events into the default calendar', async () => {
    const id = await createDeviceEvent({ accountId: 'acc-1', title: 'Diş hekimi', startAt: '2030-09-06T08:00:00Z', endAt: '2030-09-06T09:00:00Z', location: 'Kadıköy' });
    expect(id).toBe('evt-new');
    expect(Calendar.createEventAsync).toHaveBeenCalledWith('cal-1', expect.objectContaining({ title: 'Diş hekimi', location: 'Kadıköy', allDay: false }));
  });
});
