...
function ctx(opts: Partial<BuildSnapshotOptions>): { locale: Locale; timezone: string } {
  ...
  return {
    locale: opts.locale ?? prefs?.locale ?? 'tr',
    timezone: opts.timezone ?? prefs?.timezone ?? deviceTz,
  };
}
...
export function buildWidgetSnapshot(
  ...
  const itemCount =
    briefing?.highlightNumber ??
    activePriorities.length + activeMeetings.length + activeDeadlines.length + activeLife.length;

  const priorities: WidgetPriority[] = activePriorities
    .slice(0, MAX_WIDGET_PRIORITIES)
    .map((i, index) => ({
      id: i.id,
  ...
  const upcomingEvents: WidgetNextEvent[] = activeMeetings
    .filter(
      (i) => i.entityType === 'calendar_event' && i.dueAt && Date.parse(i.dueAt) > now.getTime(),
    )
  ...
  const followUpInsights = [...activePriorities, ...activeDeadlines].filter(
    (i) => i.kind === 'follow_up' || i.badge === 'follow_up',
  );
  ...
  const followUp: WidgetFollowUp | null =
    openFollowUps > 0
      ? {
          title:
            counts || !first
              ? `${openFollowUps} ${lower(t('widgets.openFollowUps'), c.locale)}`
              : first.title,
  ...
  const headline =
    briefing?.headline ??
    (itemCount > 0 ? t('today.headline', { count: itemCount }) : t('today.headlineZero'));
...
export function snapshotToNextImportantProps(s: WidgetSnapshot): NextImportantProps {
  ...
  const inlineLabel =
    s.itemCount === 0 ? t('widgets.allClear') : first ? `${countLabel} · ${first}` : countLabel;
  ...
  return {
    ...
    item: top
      ? {
          ...
          meta:
            top.timeLabel ??
            (s.generatedAtLabel
              ? `${t('widgets.brief')} · ${s.generatedAtLabel}`
              : t('common.today')),
    ...
    rectangular: s.nextEvent
      ? {
          kicker: `${kicker} · ${s.nextEvent.time}`,
          title: s.nextEvent.title,
          sub: s.nextEvent.sub,
          deepLink: s.nextEvent.deepLink,
        }
      : top
...
function splitHeadline(
  ...
  return {
    ...
    highlight: token,
    after: headline.slice(idx + token.length),
  };
}
...
export function snapshotToDailyBriefProps(s: WidgetSnapshot): DailyBriefProps {
  ...
  const briefingPath = s.briefingKind
    ? DeepLinks.briefing(s.briefingKind, s.briefingId ?? undefined)
    : DeepLinks.today();
  ...
  return {
    ...
    listenLabel:
      s.audioDurationMin && s.briefingKind
        ? t('today.listen', { minutes: s.audioDurationMin })
        : null,
    briefingUrl: widgetSchemeUrl(
      s.briefingKind
        ? `${briefingPath}${briefingPath.includes('?') ? '&' : '?'}autoplay=1`
        : briefingPath,
    ),
    ...
    followUp: s.followUp
      ? { title: s.followUp.title, sub: s.followUp.sub, deepLink: s.followUp.deepLink }
      : null,
    emptyTitle: t('widgets.allClear'),
    signedOutTitle: t('widgets.signInHint'),
    todayUrl: todayUrl(),
...
export function buildSnapshotTimeline(
  ...
  snapshot.upcomingEvents.forEach((event, index) => {
    ...
    entries.push({
      date: rollover,
      snapshot: { ...snapshot, upcomingEvents: remaining, nextEvent: remaining[0] ?? null },
    });
  });
  return entries;
}
...
function getWidgetHandles(): WidgetHandles | null {
  ...
  try {
    ...
    const next = require('../../widgets/NextImportant') as {
      NextImportantWidget: typeof NextImportantLayout;
    };
    const today = require('../../widgets/TodayPriorities') as {
      TodayPrioritiesWidget: typeof TodayPrioritiesLayout;
    };
    const brief = require('../../widgets/DailyBrief') as {
      DailyBriefWidget: typeof DailyBriefLayout;
    };
    handles = {
      nextImportant: widgets.createWidget<NextImportantProps>(
        WIDGET_NAMES.nextImportant,
        next.NextImportantWidget,
      ),
      todayPriorities: widgets.createWidget<TodayPrioritiesProps>(
        WIDGET_NAMES.todayPriorities,
        today.TodayPrioritiesWidget,
      ),
      dailyBrief: widgets.createWidget<DailyBriefProps>(
        WIDGET_NAMES.dailyBrief,
...
function toEntries<P extends object>(
  timeline: SnapshotTimelineEntry[],
  map: (s: WidgetSnapshot) => P,
): WidgetTimelineEntry<P>[] {
...
export async function syncWidgetsFromToday(
  today: TodayFeed,
  signedIn: boolean,
  opts: Omit<BuildSnapshotOptions, 'signedIn'> = {},
): Promise<WidgetSnapshot> {
  const snapshot = buildWidgetSnapshot(today, {
    signedIn,
    privacy: opts.privacy ?? currentLockScreenPrivacy(),
    now: opts.now,
