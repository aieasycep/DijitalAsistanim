...
Deno.serve(
  handler(async (req) => {
    ...
    const { data: prefs } = await admin.from('user_preferences').select('android_notification_upload_consent, android_notification_scope, android_allowed_packages').eq('user_id', user.id).maybeSingle();
    const p = prefs as { android_notification_upload_consent: boolean; android_notification_scope: 'all_allowed' | 'selected'; android_allowed_packages: string[] } | null;
    ...
    const accepted = items.filter((it) => {
      if (isSensitiveNotification({ packageName: it.packageName, title: it.title, text: it.text })) return false;
    ...
    const { error, data } = await admin.from('android_notifications').upsert(rows, { onConflict: 'user_id,fingerprint', ignoreDuplicates: true }).select('id');
    if (error) throw new AppError('internal', `Bildirimler kaydedilemedi: ${error.message}`);
    await audit(admin, { userId: user.id, action: 'notification.access_change', actor: 'user', targetType: 'android_notifications', metadata: { accepted: accepted.length, rejected: items.length - accepted.length } });
