...
import { adminClient, assertMethod, handler, json, parseInput, requireUser } from '../_shared/mod.ts';

type Db = ReturnType<typeof adminClient>;

async function loadTarget(db: Db, userId: string, type: string, id: string, dueAtOverride: string | null | undefined): Promise<ReminderTarget> {
  ...
  switch (type) {
    ...
    case 'email_thread': {
      const { data } = await db.from('email_threads').select('analysis').eq('id', id).eq('user_id', userId).maybeSingle();
      if (!data) throw new AppError('not_found', 'Hatırlatıcı hedefi bulunamadı.');
    ...
    case 'task': {
      ...
      return { dueAt: dueAtOverride ?? r.due_at ?? null, startAt: r.scheduled_start_at ?? null, isMeeting: false };
    }
    case 'commitment': {
    ...
    case 'follow_up': {
      ...
      const due = r.snoozed_until ?? (r.sent_at ? new Date(Date.parse(r.sent_at) + Number(r.nudge_after_days ?? 3) * 86_400_000).toISOString() : null);
...
Deno.serve(
  handler(async (req) => {
    ...
    const [target, { data: prefs }, { data: notif }, { data: events }] = await Promise.all([
      ...
      admin.from('user_preferences').select('locale, timezone').eq('user_id', user.id).maybeSingle(),
      admin.from('notification_preferences').select('quiet_hours_enabled, quiet_hours_start, quiet_hours_end, meeting_lead_minutes').eq('user_id', user.id).maybeSingle(),
      admin
    ...
    const n = notif as { quiet_hours_enabled: boolean; quiet_hours_start: string; quiet_hours_end: string; meeting_lead_minutes: number } | null;
    ...
    const response: SmartReminderSuggestResponse = computeReminderOptions({
      ...
      locale: p?.locale ?? 'tr',
      busy: ((events ?? []) as { start_at: string; end_at: string }[]).map((e) => ({ startAt: e.start_at, endAt: e.end_at })),
      quietHours: n ? { enabled: n.quiet_hours_enabled, start: n.quiet_hours_start, end: n.quiet_hours_end } : null,
