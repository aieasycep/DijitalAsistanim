...
import { adminClient, assertMethod, enforceRateLimit, handler, json, requireUser } from '../_shared/mod.ts';
...
Deno.serve(
  handler(async (req) => {
    ...
    if (!contentType.includes('multipart/form-data')) throw new AppError('validation', 'Ses dosyası multipart olarak gönderilmeli.');
    const form = await req.formData();
    const file = form.get('file');
    if (!(file instanceof File)) throw new AppError('validation', 'Ses dosyası eksik.');
    if (file.size === 0 || file.size > MAX_BYTES) throw new AppError('validation', 'Ses dosyası boyutu desteklenmiyor.');
    const durationSec = Math.min(MAX_DURATION_SEC, Math.max(0, Number(form.get('durationSec') ?? 0) || 0));
    const mimeType = String(form.get('mimeType') ?? file.type ?? 'audio/m4a');

    const admin = adminClient();
    const { data: prefs } = await admin.from('user_preferences').select('locale, timezone').eq('user_id', user.id).maybeSingle();
    ...
    if (seconds > 0) {
      ...
      const { data } = await admin.from('usage_counters').select('voice_seconds').eq('user_id', user.id).eq('day', day).maybeSingle();
      const current = (data as { voice_seconds: number } | null)?.voice_seconds ?? 0;
      await admin.from('usage_counters').upsert({ user_id: user.id, day, voice_seconds: current + seconds, updated_at: new Date().toISOString() }, { onConflict: 'user_id,day' });
    }
