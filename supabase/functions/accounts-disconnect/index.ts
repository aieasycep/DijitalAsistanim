...
Deno.serve(
  handler(async (req) => {
    ...
    const { data: account, error } = await admin.from('connected_accounts').select('id, provider, user_id').eq('id', accountId).eq('user_id', user.id).maybeSingle();
    ...
    await admin
      ...
      .update({ status: 'disconnected', deleted_at: new Date().toISOString(), is_primary: false, last_error: null })
      .eq('id', accountId);

    await audit(admin, { userId: user.id, action: 'oauth.revoke', actor: 'user', targetType: 'connected_account', targetId: accountId, metadata: { revoked: result.revoked } });

    return json({ ok: true as const, revoked: result.revoked, ...(result.manualUrl ? { manualUrl: result.manualUrl } : {}) });
  }),
);
