...
import { resolveSttProvider, resolveTtsProvider, type SttProvider, type TtsProvider } from '@da/server-core/speech';
...
function cacheStore(admin: Db, userId: string): AiCacheStore {
  return {
    async get(key) {
      const { data } = await admin.from('ai_analysis_cache').select('result, created_at').eq('user_id', userId).eq('fingerprint', key).eq('purpose', 'client').maybeSingle();
    ...
    async set(key, value, ttlSec) {
      ...
      await admin.from('ai_analysis_cache').upsert({ user_id: userId, fingerprint: key, purpose: 'client', result: { value, expiresAt } }, { onConflict: 'user_id,fingerprint,purpose' });
    },
  };
}
...
async function recordUsage(admin: Db, timezone: string, r: AiUsageRecord): Promise<void> {
  ...
  if (tokens > 0 && !r.cached) {
    ...
    const { data } = await admin.from('usage_counters').select('ai_tokens').eq('user_id', r.userId).eq('day', day).maybeSingle();
    const current = (data as { ai_tokens: number } | null)?.ai_tokens ?? 0;
    await admin.from('usage_counters').upsert({ user_id: r.userId, day, ai_tokens: current + tokens, updated_at: new Date().toISOString() }, { onConflict: 'user_id,day' });
  }
}
...
export function createAi(ctx: AiContext): AiClient {
  ...
  if (!aiConfigured()) throw new AppError('ai_unavailable', 'AI sağlayıcısı bu ortamda yapılandırılmamış.', { status: 503 });
  const provider = env.ai.provider === 'anthropic' && !env.ai.anthropicApiKey ? 'openai' : env.ai.provider === 'openai' && !env.ai.openaiApiKey ? 'anthropic' : env.ai.provider;
  return createAiClient({
    provider,
    fallbackProvider: env.ai.fallbackProvider ?? null,
    ...(env.ai.anthropicApiKey ? { anthropic: { apiKey: env.ai.anthropicApiKey, modelSmall: env.ai.anthropicModelSmall, modelLarge: env.ai.anthropicModelLarge } } : {}),
    ...(env.ai.openaiApiKey ? { openai: { apiKey: env.ai.openaiApiKey, modelSmall: env.ai.openaiModelSmall, modelLarge: env.ai.openaiModelLarge } } : {}),
...
export async function checkAiBudget(ctx: AiContext, requested = 0): Promise<AiBudgetStatus> {
  ...
  const { data } = await admin.from('usage_counters').select('ai_tokens').eq('user_id', ctx.userId).eq('day', day).maybeSingle();
...
export function createEmbeddings(): EmbeddingProvider | null {
  const env = getEnv();
  const apiKey = env.embeddings.provider === 'voyage' ? env.embeddings.voyageApiKey : env.ai.openaiApiKey;
