...
import { adminClient, assertMethod, audit, getEnv, handler, json, parseInput } from '../_shared/mod.ts';
...
Deno.serve(
  handler(async (req) => {
    ...
    const env = getEnv();
    if (!env.revenuecat.webhookSecret) throw new AppError('provider_unavailable', 'RevenueCat webhook secret yapılandırılmamış.', { status: 503 });
    ...
    const { error: dupErr } = await admin.from('webhook_events').insert({ id: eventKey, source: 'revenuecat' });
    if (dupErr?.code === '23505') return json({ ok: true as const, duplicate: true });

    const candidates = [event.app_user_id, event.original_app_user_id].filter((v): v is string => Boolean(v));
    const { data: profile } = await admin.from('profiles').select('id').or(candidates.map((c) => `revenuecat_app_user_id.eq.${c},id.eq.${c}`).join(',')).limit(1).maybeSingle();
    ...
    if (!userId) {
      ...
      await admin.from('webhook_events').update({ processed_at: new Date().toISOString() }).eq('id', eventKey);
      return json({ ok: true as const, linked: false });
    }

    const { data: existingRow } = await admin.from('subscriptions').select('*').eq('user_id', userId).eq('source', 'revenuecat').maybeSingle();
    ...
    if (result.changed) {
      ...
      const grantsPro = (s.status === 'active' || s.status === 'trial' || s.status === 'grace') && (!s.expiresAt || Date.parse(s.expiresAt) > Date.now());
      await admin.from('profiles').update({ plan: grantsPro ? 'pro' : 'free', revenuecat_app_user_id: event.app_user_id }).eq('id', userId);
      await audit(admin, {
        userId,
        action: 'subscription.change',
        actor: 'webhook',
        targetType: 'subscription',
        metadata: { eventType: result.eventType, status: s.status, sandbox: result.sandbox, transferred: result.transferred },
      });
    }
    await admin.from('webhook_events').update({ processed_at: new Date().toISOString() }).eq('id', eventKey);
