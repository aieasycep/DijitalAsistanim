...
function fallbackQuestions(ctx: Context, locale: 'tr' | 'en'): SuggestedQuestionsResponse {
  ...
  if (ctx.contact) {
    ...
    q.push({
      text: tr ? `${n} ile son konuştuğumuz konu neydi?` : `What did I last discuss with ${n}?`,
    });
    if (ctx.contact.userOwes.length)
      q.push({
        text: tr ? `${n}'e ne sözü verdim?` : `What did I promise ${n}?`,
        reason: tr ? 'Açık sözler var' : 'Open promises',
      });
    if (ctx.contact.theyOwe.length)
      q.push({
        text: tr ? `${n} bana ne göndermeyi bekliyorum?` : `What am I waiting on from ${n}?`,
      });
    q.push({
      text: tr ? `${n} ile bir sonraki görüşmem ne zaman?` : `When is my next meeting with ${n}?`,
    });
  } else {
    q.push({ text: tr ? 'Bugün önce ne yapmalıyım?' : 'What should I do first today?' });
    if (ctx.counts.deadlines > 0)
      q.push({
        text: tr ? 'Bu hafta hangi son tarihler var?' : 'Which deadlines are due this week?',
        reason: tr ? `${ctx.counts.deadlines} son tarih` : `${ctx.counts.deadlines} deadlines`,
      });
    if (ctx.counts.followUps > 0)
      q.push({
        text: tr ? 'Kimden yanıt bekliyorum?' : 'Who am I waiting on?',
        reason: tr ? `${ctx.counts.followUps} takip` : `${ctx.counts.followUps} follow-ups`,
      });
    const ev = ctx.upcomingEvents[0];
    if (ev)
      q.push({
        text: tr
          ? `${ev.title} toplantısına nasıl hazırlanmalıyım?`
          : `How should I prepare for ${ev.title}?`,
      });
    const person = ctx.topPeople[0];
    if (person)
      q.push({
        text: tr ? `${person.name} ile açık konular neler?` : `What is open with ${person.name}?`,
      });
  ...
  return {
    questions: q
      .slice(0, 6)
      .map((item, i) => ({ id: `fallback-${i}`, text: item.text, reason: item.reason ?? null })),
  };
}
...
Deno.serve(
  handler(async (req) => {
    ...
    const [
      ...
      db
        .from('email_threads')
        .select('subject, participants, importance, analysis, last_message_at')
        .eq('user_id', user.id)
        .is('deleted_at', null)
      ...
      db
        ...
        .limit(20),
      contactId
        ? db.from('contacts').select('*').eq('id', contactId).eq('user_id', user.id).maybeSingle()
        : Promise.resolve({ data: null }),
    ]);

    const threadList = camelize<
      Pick<EmailThread, 'subject' | 'participants' | 'importance' | 'analysis' | 'lastMessageAt'>[]
    >(threads ?? []);
    const eventList = camelize<Pick<CalendarEvent, 'title' | 'startAt' | 'attendees'>[]>(
      events ?? [],
    );
    ...
    for (const t of threadList)
      for (const p of t.participants)
        if (p.name) peopleCount.set(p.name, (peopleCount.get(p.name) ?? 0) + 1);
    const deadlines = threadList.filter(
      (t) => t.analysis?.deadline && Date.parse(t.analysis.deadline) <= Date.parse(in7d),
    ).length;
    const contactCommitments = contact
      ? commitmentList.filter((c) => c.counterpartContactId === contact.id)
      : [];
    const contactThread = contact
      ? threadList.find((t) =>
          t.participants.some((p) => p.email && contact.emails.includes(p.email)),
        )
      : undefined;

    const ctx: Context = {
      counts: {
        importantEmails: threadList.filter(
      ...
      topPeople: [...peopleCount.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
        .map(([name, count]) => ({ name, count })),
      upcomingEvents: eventList.map((e) => ({
        title: e.title,
        at: e.startAt,
        with: e.attendees.find((a) => !a.isOrganizer)?.name ?? null,
      })),
      recentTopics: threadList
        .slice(0, 8)
        .map((t) => t.subject)
        .filter(Boolean),
      openLoops: [
        ...((followUps ?? []) as { counterpart_name: string; topic: string }[]).map(
          (f) => `${f.counterpart_name}: ${f.topic}`,
        ),
      ...
      contact: contact
        ? {
            ...
            lastContact: contactThread
              ? {
                  at: contactThread.lastMessageAt,
                  summary: contactThread.analysis?.summary ?? contactThread.subject,
                }
              : null,
            userOwes: contactCommitments
              .filter((c) => c.direction === 'user_owes')
              .map((c) => c.text),
            theyOwe: contactCommitments
    ...
    try {
      ...
      const cacheKey = await sha256Hex(
        `questions:${user.id}:${contactId ?? 'today'}:${dayKey}:${ctx.counts.importantEmails}:${ctx.counts.events}:${ctx.openLoops.length}`,
      );
      const result = await createAi(aiCtx).generateStructured(suggestedQuestionsAiSchema, spec, {
        userId: user.id,
        locale: planInfo.locale,
        cacheKey,
        cacheTtlSec: 3600,
      ...
      log.warn('suggested questions fell back to rules', {
